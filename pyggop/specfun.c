/* specfun.f -- translated by f2c (version 20000121).
...
/* This is to avoid the "missing MAIN__" error when linking */
...
static doublecomplex c_b83 = {.5,0.};
static doublecomplex c_b107 = {-.125,0.};
static doublecomplex c_b111 = {.375,0.};
static integer c_n1 = -1;
static integer c__200 = 200;
static integer c__15 = 15;
static integer c__3 = 3;
static doublereal c_b199 = 3.;
static doublereal c_b285 = .33333333333333331;
static real c_b381 = (float)2.;
static doublereal c_b387 = 1.27;
static integer c__9 = 9;
static integer c__1 = 1;
static integer c__0 = 0;
static doublereal c_b597 = 1.;
static doublecomplex c_b1300 = {-2.,0.};
static doublereal c_b1307 = .33333;
static doublecomplex c_b1525 = {-1.,0.};
...
/*      Changed according to ERRATA also. */
...
/* Subroutine */ int cpdsa_(n, z__, cdn)
integer *n;
doublecomplex *z__, *cdn;
{
    /* System generated locals */
...
/*       =========================================================== */
	...
	if (z_abs(z__) == (float)0.) {
	    if (va0 <= (float)0. && va0 == (doublereal) ((integer) va0)) {
		cdn->r = 0., cdn->i = 0.;
	    } else {
		gaih_(&va0, &ga0);
		d__1 = *n * -.5;
...
/*       ********************************** */
/* Subroutine */ int cfs_(z__, zf, zd)
doublecomplex *z__, *zf, *zd;
{
    /* System generated locals */
...
/* L10: */
    ...
    } else if (w0 > (float)2.5 && w0 < (float)4.5) {
	m = 85;
	s.r = z0.r, s.i = z0.i;
...
/*       ********************************** */
/* Subroutine */ int lqmn_(mm, m, n, x, qm, qd)
integer *mm, *m, *n;
doublereal *x, *qm, *qd;
{
    /* System generated locals */
...
/* L20: */
	...
	if (abs(*x) > 1.1) {
	    ...
	    km = (*m + 40 + *n) * (integer) ((float)-1. - log(*x - (float)1.) 
		    * (float)1.8);
	}
	qf2 = 0.;
	qf1 = 1.;
...
/*       ********************************** */
/* Subroutine */ int clpmn_(mm, m, n, x, y, ntype, cpm, cpd)
integer *mm, *m, *n;
doublereal *x, *y;
integer *ntype;
doublecomplex *cpm, *cpd;
{
    /* System generated locals */
...
doublereal *va, *x, *pv;
{
    ...
    /* Builtin functions */
    double exp(), sin(), pow_dd(), sqrt();

    /* Local variables */
    static integer m;
    static doublereal r__, a0, g1, r1;
    extern /* Subroutine */ int gamma2_();
    static doublereal v1, gm, ep, pi, gw, vm, sv, ga0, va0, vb0, sq2, sv0, 
	    fac, eps;
...
/*       =================================================== */
    ...
    va0 = *va * .5 + 1.;
    if (*x == (float)0.) {
	if (va0 <= (float)0. && va0 == (doublereal) ((integer) va0) || *va == 
		(float)0.) {
	    *pv = 0.;
	} else {
	    vb0 = *va * -.5;
	...
	*pv = (sv + 1.) * g1;
	...
	for (m = 1; m <= 250; ++m) {
	    ...
	    r1 = gw * r__ * gm;
	    *pv += r1;
	    if ((d__1 = r1 / *pv, abs(d__1)) < eps && gw != (float)0.) {
		goto L15;
	    }
/* L10: */
	}
...
/* Subroutine */ int jdzo_(nt, n, m, p, zo)
integer *nt, *n, *m, *p;
doublereal *zo;
{
    /* System generated locals */
...
/*       ============================================================= */
	...
	d__1 = (doublereal) (*nt);
	d__2 = (doublereal) (*nt);
	xm = pow_dd(&d__1, &c_b50) * (float)2.248485 - (float)1. - *nt * (
		float).0159382 + pow_dd(&d__2, &c_b51) * (float)3.208775e-4;
	nm = (integer) (*nt * (float).05875 + (float)14.5);
	mm = (integer) (*nt * (float).02) + 6;
    } else {
	d__1 = (doublereal) (*nt);
	d__2 = (doublereal) (*nt);
	xm = pow_dd(&d__1, &c_b50) * (float)1.445389 + (float)5. + *nt * (
		float).01889876 - pow_dd(&d__2, &c_b51) * (float)2.147763e-4;
	nm = (integer) (*nt * (float).0327 + (float)27.8);
	mm = (integer) (*nt * (float).01088) + 10;
    }
    l0 = 0;
    i__1 = nm;
    for (i__ = 1; i__ <= i__1; ++i__) {
	d__1 = (doublereal) (i__ - 1);
	x1 = pow_dd(&d__1, &c_b50) * (float).4795504 + (float).407658 + (i__ 
		- 1) * (float).983618;
	d__1 = (doublereal) (i__ - 1);
	x2 = pow_dd(&d__1, &c_b50) * (float).8333883 + (float)1.99535 + (i__ 
		- 1) * (float).984584;
	l1 = 0;
	i__2 = mm;
	for (j = 1; j <= i__2; ++j) {
...
/* Computing 2nd power */
		i__3 = j + 1;
		x1 = x + (float)3.057 + (i__ - 1) * (float).0122 + ((i__ - 1) 
			* (float).41575 + (float)1.555) / (i__3 * i__3);
	    } else {
/* Computing 2nd power */
		i__3 = j + 1;
		x1 = x + (float)2.918 + (i__ - 1) * (float).01924 + ((i__ - 1)
			 * (float).13205 + (float)6.26) / (i__3 * i__3);
	    }
L20:
	    x = x2;
L25:
...
/* Computing 2nd power */
		i__3 = j + 1;
		x2 = x + (float)3.11 + (i__ - 1) * (float).0138 + ((i__ - 1) *
			 (float).2804 + (float).04832) / (i__3 * i__3);
	    } else {
/* Computing 2nd power */
		i__3 = j + 3;
		x2 = x + (float)3.001 + (i__ - 1) * (float).0105 + ((i__ - 1) 
			* (float).48525 + (float)11.52) / (i__3 * i__3);
	    }
L30:
	    ;
	}
...
/*       ********************************** */
/* Subroutine */ int cbk_(m, n, c__, cv, qt, ck, bk)
integer *m, *n;
doublereal *c__, *cv, *qt, *ck, *bk;
{
    /* System generated locals */
    integer i__1, i__2, i__3;
    doublereal d__1;
...
/* L10: */
    ...
    for (j = 1; j <= i__1; ++j) {
/* L15: */
	v[j - 1] = (j * (float)2. - (float)1. - ip) * ((j - *m) * (float)2. - 
		ip) + *m * (*m - (float)1.) - *cv;
    }
    i__1 = nm - 1;
    for (j = 1; j <= i__1; ++j) {
/* L20: */
	w[j - 1] = (j * (float)2. - ip) * (j * (float)2. + (float)1. - ip);
    }
    if (ip == 0) {
	sw = 0.;
	i__1 = n2 - 1;
...
/*       ********************************** */
/* Subroutine */ int cjy01_(z__, cbj0, cdj0, cbj1, cdj1, cby0, cdy0, cby1, 
	cdy1)
doublecomplex *z__, *cbj0, *cdj0, *cbj1, *cdj1, *cby0, *cdy0, *cby1, *cdy1;
{
    /* Initialized data */
    ...
    /* Builtin functions */
    double z_abs();
    void z_log(), z_div(), pow_zi(), z_sqrt(), z_cos(), z_sin();
    double d_imag();
...
L25:
	...
	z__1.r = z__2.r * cbj1->r - z__2.i * cbj1->i, z__1.i = z__2.r * 
		cbj1->i + z__2.i * cbj1->r;
	cbj1->r = z__1.r, cbj1->i = z__1.i;
	w0 = 0.;
	cr.r = 1., cr.i = 0.;
	cs.r = 0., cs.i = 0.;
...
L45:
	...
	z__4.r = z__5.r * cbj1->r - z__5.i * cbj1->i, z__4.i = z__5.r * 
		cbj1->i + z__5.i * cbj1->r;
	z_div(&z__8, &c_b6, &z1);
...
/* L65: */
    ...
    z__1.r = -cbj1->r, z__1.i = -cbj1->i;
    cdj0->r = z__1.r, cdj0->i = z__1.i;
    z_div(&z__3, &c_b6, z__);
    z__2.r = z__3.r * cbj1->r - z__3.i * cbj1->i, z__2.i = z__3.r * cbj1->i + 
	    z__3.i * cbj1->r;
    z__1.r = cbj0->r - z__2.r, z__1.i = cbj0->i - z__2.i;
    cdj1->r = z__1.r, cdj1->i = z__1.i;
...
/*       ********************************** */
/* Subroutine */ int rmn2sp_(m, n, c__, x, cv, df, kd, r2f, r2d)
integer *m, *n;
doublereal *c__, *x, *cv, *df;
integer *kd;
doublereal *r2f, *r2d;
{
    /* System generated locals */
    ...
    /* Local variables */
    static integer j, k;
    extern /* Subroutine */ int lpmns_(), lqmns_();
    static integer j1, j2, l1;
...
/* L40: */
	...
	i__2 = j;
	for (l1 = 1; l1 <= i__2; ++l1) {
	    r3 = r3 * .5 * (-j + l1 - (float)1.) * (j + l1) / ((*m + l1) * l1)
		     * ((float)1. - *x);
/* L45: */
	    sf += r3;
	}
	...
	spd1 = *m / (*x * *x - 1.) * spl;
	gc = j * .5 * (j + (float)1.) / (*m + (float)1.);
	sd = 1.;
	r4 = 1.;
	i__2 = j - 1;
	for (l1 = 1; l1 <= i__2; ++l1) {
	    r4 = r4 * .5 * (-j + l1) * (j + l1 + (float)1.) / ((*m + l1 + (
		    float)1.) * l1) * ((float)1. - *x);
/* L50: */
	    sd += r4;
	}
	spd2 = r1 * ga * gb * gc * sd;
...
/*       ********************************** */
/* Subroutine */ int qstar_(m, n, c__, ck, ck1, qs, qt)
integer *m, *n;
doublereal *c__, *ck, *ck1, *qs, *qt;
{
    /* System generated locals */
    integer i__1, i__2, i__3;
    doublereal d__1;
...
/*       ********************************** */
/* Subroutine */ int cv0_(kd, m, q, a0)
integer *kd, *m;
doublereal *q, *a0;
{
    extern /* Subroutine */ int cvql_(), cvqm_();
...
/*       ==================================================== */
    ...
    if (*m == 0) {
	if (*q <= (float)1.) {
	    *a0 = (((q2 * (float).0036392 - (float).0125868) * q2 + (float)
		    .0546875) * q2 - (float).5) * q2;
	} else if (*q <= (float)10.) {
	    *a0 = ((*q * .003999267 - .09638957) * *q - (float).88297) * *q + 
		    (float).5542818;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= (float)1. && *kd == 2) {
	    *a0 = (((*q * (float)-6.51e-4 - (float).015625) * *q - (float)
		    .125) * *q + (float)1.) * *q + (float)1.;
	} else if (*q <= (float)1. && *kd == 3) {
	    *a0 = (((*q * (float)-6.51e-4 + (float).015625) * *q - (float)
		    .125) * *q - (float)1.) * *q + (float)1.;
	} else if (*q <= (float)10. && *kd == 2) {
	    *a0 = (((*q * -4.94603e-4 + .0192917) * *q - (float).3089229) * *
		    q + (float)1.33372) * *q + (float).811752;
	} else if (*q <= (float)10. && *kd == 3) {
	    *a0 = ((*q * .001971096 - .05482465) * *q - (float)1.152218) * *q 
		    + (float)1.10427;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= (float)1. && *kd == 1) {
	    *a0 = (((q2 * (float)-.0036391 + (float).0125888) * q2 - (float)
		    .0551939) * q2 + (float).416667) * q2 + (float)4.;
	} else if (*q <= (float)1. && *kd == 4) {
	    *a0 = (q2 * (float)3.617e-4 - (float).0833333) * q2 + (float)4.;
	} else if (*q <= 15. && *kd == 1) {
	    *a0 = (((*q * 3.200972e-4 - .008667445) * *q - 1.829032e-4) * *q 
		    + (float).9919999) * *q + (float)3.3290504;
	} else if (*q <= (float)10. && *kd == 4) {
	    *a0 = ((*q * .00238446 - (float).08725329) * *q - .004732542) * *
		    q + (float)4.00909;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= (float)1. && *kd == 2) {
	    *a0 = ((*q * (float)6.348e-4 + (float).015625) * *q + (float)
		    .0625) * q2 + (float)9.;
	} else if (*q <= (float)1. && *kd == 3) {
	    *a0 = ((*q * (float)6.348e-4 - (float).015625) * *q + (float)
		    .0625) * q2 + (float)9.;
	} else if (*q <= (float)20. && *kd == 2) {
	    *a0 = (((*q * 3.035731e-4 - .01453021) * *q + (float).19069602) * 
		    *q - (float).1039356) * *q + (float)8.9449274;
	} else if (*q <= (float)15. && *kd == 3) {
	    *a0 = ((*q * 9.369364e-5 - (float).03569325) * *q + (float)
		    .2689874) * *q + (float)8.771735;
	} else {
	    cvql_(kd, m, q, a0);
	}
    } else if (*m == 4) {
	if (*q <= (float)1. && *kd == 1) {
	    *a0 = ((q2 * (float)-2.1e-6 + (float)5.012e-4) * q2 + (float)
		    .0333333) * q2 + (float)16.;
	} else if (*q <= (float)1. && *kd == 4) {
	    *a0 = ((q2 * (float)3.7e-6 - (float)3.669e-4) * q2 + (float)
		    .0333333) * q2 + (float)16.;
	} else if (*q <= (float)25. && *kd == 1) {
	    *a0 = (((*q * 1.076676e-4 - .0079684875) * *q + (float).17344854) 
		    * *q - (float).5924058) * *q + (float)16.620847;
	} else if (*q <= (float)20. && *kd == 4) {
	    *a0 = ((*q * -7.08719e-4 + .0038216144) * *q + (float).1907493) * 
		    *q + (float)15.744;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= (float)1. && *kd == 2) {
	    *a0 = ((*q * (float)6.8e-6 + (float)1.42e-5) * q2 + (float)
		    .0208333) * q2 + (float)25.;
	} else if (*q <= (float)1. && *kd == 3) {
	    *a0 = ((*q * (float)-6.8e-6 + (float)1.42e-5) * q2 + (float)
		    .0208333) * q2 + (float)25.;
	} else if (*q <= (float)35. && *kd == 2) {
	    *a0 = (((*q * 2.238231e-5 - .002983416) * *q + (float).10706975) *
		     *q - (float).600205) * *q + (float)25.93515;
	} else if (*q <= (float)25. && *kd == 3) {
	...
	if (*q <= (float)1.) {
	    *a0 = (q2 * 4e-7 + (float).0142857) * q2 + (float)36.;
	} else if (*q <= (float)40. && *kd == 1) {
	    *a0 = (((*q * -1.66846e-5 + 4.80263e-4) * *q + .0253998) * *q - (
		    float).181233) * *q + (float)36.423;
	} else if (*q <= (float)35. && *kd == 4) {
	...
	if (*q <= (float)10.) {
	    cvqm_(m, q, a0);
	} else if (*q <= (float)50. && *kd == 2) {
	    *a0 = (((*q * -1.411114e-5 + 9.730514e-4) * *q - .003097887) * *q 
		    + .03533597) * *q + (float)49.0547;
	} else if (*q <= (float)40. && *kd == 3) {
	...
	if (*q <= *m * (float)3.) {
	    ...
	    if (*m == 8 && *kd == 1) {
		*a0 = (((*q * 8.634308e-6 - .002100289) * *q + (float).169072)
			 * *q - (float)4.64336) * *q + (float)109.4211;
	    } else if (*m == 8 && *kd == 4) {
		*a0 = ((*q * -6.7842e-5 + .0022057) * *q + (float).48296) * *
			q + (float)56.59;
	    } else if (*m == 9 && *kd == 2) {
		*a0 = (((*q * 2.906435e-6 - .001019893) * *q + (float)
			.1101965) * *q - (float)3.821851) * *q + (float)
			127.6098;
	    } else if (*m == 9 && *kd == 3) {
		*a0 = ((*q * -9.577289e-5 + (float).01043839) * *q + (float)
			.06588934) * *q + (float)78.0198;
	    } else if (*m == 10 && *kd == 1) {
		*a0 = (((*q * 5.44927e-7 - 3.926119e-4) * *q + (float)
			.0612099) * *q - (float)2.600805) * *q + (float)
			138.1923;
	    } else if (*m == 10 && *kd == 4) {
		*a0 = ((*q * -7.660143e-5 + (float).01132506) * *q - (float)
			.09746023) * *q + (float)99.29494;
	    } else if (*m == 11 && *kd == 2) {
		*a0 = (((*q * -5.67615e-7 + 7.152722e-6) * *q + (float)
			.01920291) * *q - (float)1.081583) * *q + (float)
			140.88;
	    } else if (*m == 11 && *kd == 3) {
		*a0 = ((*q * -6.310551e-5 + (float).0119247) * *q - (float)
			.2681195) * *q + (float)123.667;
	    } else if (*m == 12 && *kd == 1) {
		*a0 = (((*q * -2.38351e-7 - 2.90139e-5) * *q + (float)
			.02023088) * *q - (float)1.289) * *q + (float)
			171.2723;
	    } else if (*m == 12 && *kd == 4) {
		*a0 = (((*q * 3.08902e-7 - 1.577869e-4) * *q + (float)
			.0247911) * *q - (float)1.05454) * *q + (float)
			161.471;
	    }
	}
    }
...
/*       ===================================================== */
    ...
    hm1 = *q * (float).5 / (*m * *m - (float)1.);
/* Computing 3rd power */
    d__1 = hm1;
    hm3 = d__1 * (d__1 * d__1) * (float).25 / (*m * *m - (float)4.);
    hm5 = hm1 * hm3 * *q / ((*m * *m - (float)1.) * (*m * *m - (float)9.));
/* Computing 4th power */
    i__1 = *m, i__1 *= i__1;
    *a0 = *m * *m + *q * (hm1 + (*m * (float)5. * *m + (float)7.) * hm3 + (
	    i__1 * i__1 * (float)9. + *m * (float)58. * *m + (float)29.) * 
	    hm5);
    return 0;
} /* cvqm_ */

/*       ********************************** */
/* Subroutine */ int cvql_(kd, m, q, a0)
integer *kd, *m;
doublereal *q, *a0;
{
    /* Builtin functions */
...
/*       ======================================================== */
    ...
    w4 = w2 * w2;
    w6 = w2 * w4;
    d1 = (float)34. / w2 + (float)5. + (float)9. / w4;
    d2 = ((float)410. / w2 + (float)33. + (float)405. / w4) / w;
    d3 = ((float)1260. / w2 + (float)63. + (float)2943. / w4 + (float)486. / 
	    w6) / w2;
    d4 = ((float)15617. / w2 + (float)527. + (float)69001. / w4 + (float)
	    41607. / w6) / w3;
    c1 = (float)128.;
    p2 = *q / w4;
    p1 = sqrt(p2);
    cv1 = *q * (float)-2. + w * (float)2. * sqrt(*q) - (w2 + (float)1.) / (
	    float)8.;
    cv2 = w + (float)3. / w + d1 / (p1 * (float)32.) + d2 / (c1 * (float)8. * 
	    p2);
    cv2 = cv2 + d3 / (c1 * (float)64. * p1 * p2) + d4 / (c1 * (float)16. * c1 
	    * p2 * p2);
    *a0 = cv1 - cv2 / (c1 * p1);
    return 0;
} /* cvql_ */
...
/*       ********************************** */
/* Subroutine */ int csphjy_(n, z__, nm, csj, cdj, csy, cdy)
integer *n;
doublecomplex *z__;
integer *nm;
doublecomplex *csj, *cdj, *csy, *cdy;
{
    /* System generated locals */
    ...
    /* Builtin functions */
    double z_abs();
    void z_sin(), z_div(), z_cos(), pow_zi();

    /* Local variables */
    extern integer msta1_(), msta2_();
    static integer k, m;
    static doublereal a0;
    static doublecomplex cf, cs, cf0, cf1, csa, csb;
...
/*       ========================================================== */
    ...
    *nm = *n;
	...
	for (k = 0; k <= i__1; ++k) {
	    i__2 = k;
	    csj[i__2].r = 0., csj[i__2].i = 0.;
	    i__2 = k;
	    cdj[i__2].r = 0., cdj[i__2].i = 0.;
	    i__2 = k;
	    csy[i__2].r = -1e300, csy[i__2].i = 0.;
/* L10: */
	    i__2 = k;
	...
	csj[0].r = 1., csj[0].i = 0.;
	if (*n > 0) {
	    cdj[1].r = .333333333333333, cdj[1].i = 0.;
	}
	return 0;
    }
    z_sin(&z__2, z__);
    z_div(&z__1, &z__2, z__);
    csj[0].r = z__1.r, csj[0].i = z__1.i;
    z_cos(&z__3, z__);
    z_sin(&z__5, z__);
    z_div(&z__4, &z__5, z__);
    ...
    z_cos(&z__3, z__);
    z__2.r = csj[0].r - z__3.r, z__2.i = csj[0].i - z__3.i;
    z_div(&z__1, &z__2, z__);
    csj[1].r = z__1.r, csj[1].i = z__1.i;
    if (*n >= 2) {
	csa.r = csj[0].r, csa.i = csj[0].i;
	csb.r = csj[1].r, csb.i = csj[1].i;
	m = msta1_(&a0, &c__200);
	if (m < *n) {
	    *nm = m;
	} else {
	...
	for (k = m; k >= 0; --k) {
	    ...
	    if (k <= *nm) {
		i__2 = k;
		csj[i__2].r = cf.r, csj[i__2].i = cf.i;
	    }
	    cf0.r = cf1.r, cf0.i = cf1.i;
/* L15: */
	    cf1.r = cf.r, cf1.i = cf.i;
	}
	if (z_abs(&csa) > z_abs(&csb)) {
	    z_div(&z__1, &csa, &cf1);
	    cs.r = z__1.r, cs.i = z__1.i;
	}
	if (z_abs(&csa) <= z_abs(&csb)) {
...
/* L20: */
	    i__1 = k;
	    i__3 = k;
	    z__1.r = cs.r * csj[i__3].r - cs.i * csj[i__3].i, z__1.i = cs.r * 
		    csj[i__3].i + cs.i * csj[i__3].r;
	    csj[i__1].r = z__1.r, csj[i__1].i = z__1.i;
	}
    }
    i__1 = *nm;
    for (k = 1; k <= i__1; ++k) {
/* L25: */
	...
	d__1 = k + 1.;
	i__4 = k;
	z__3.r = d__1 * csj[i__4].r, z__3.i = d__1 * csj[i__4].i;
	z_div(&z__2, &z__3, z__);
	z__1.r = csj[i__2].r - z__2.r, z__1.i = csj[i__2].i - z__2.i;
	cdj[i__3].r = z__1.r, cdj[i__3].i = z__1.i;
    }
    z_sin(&z__3, z__);
    z__2.r = csy[0].r - z__3.r, z__2.i = csy[0].i - z__3.i;
    z_div(&z__1, &z__2, z__);
    csy[1].r = z__1.r, csy[1].i = z__1.i;
    z__3.r = cdy[0].r * 2., z__3.i = cdy[0].i * 2.;
    z_cos(&z__4, z__);
    z__2.r = z__3.r - z__4.r, z__2.i = z__3.i - z__4.i;
    z_div(&z__1, &z__2, z__);
    cdy[1].r = z__1.r, cdy[1].i = z__1.i;
    i__3 = *nm;
    for (k = 2; k <= i__3; ++k) {
	if (z_abs(&csj[k - 1]) > z_abs(&csj[k - 2])) {
	    i__2 = k;
	    i__4 = k;
	    i__1 = k - 1;
	    z__3.r = csj[i__4].r * csy[i__1].r - csj[i__4].i * csy[i__1].i, 
		    z__3.i = csj[i__4].r * csy[i__1].i + csj[i__4].i * csy[
		    i__1].r;
	    z__5.r = z__->r * z__->r - z__->i * z__->i, z__5.i = z__->r * 
		    z__->i + z__->i * z__->r;
	    z_div(&z__4, &c_b6, &z__5);
	    z__2.r = z__3.r - z__4.r, z__2.i = z__3.i - z__4.i;
	    z_div(&z__1, &z__2, &csj[k - 1]);
	    csy[i__2].r = z__1.r, csy[i__2].i = z__1.i;
	} else {
	    i__2 = k;
	    i__4 = k;
	    i__1 = k - 2;
	    ...
	    pow_zi(&z__6, z__, &c__3);
	    z_div(&z__4, &z__5, &z__6);
	    z__2.r = z__3.r - z__4.r, z__2.i = z__3.i - z__4.i;
	    z_div(&z__1, &z__2, &csj[k - 2]);
	    csy[i__2].r = z__1.r, csy[i__2].i = z__1.i;
	}
/* L30: */
    }
...
/* L35: */
	...
	d__1 = k + 1.;
	i__1 = k;
	z__3.r = d__1 * csy[i__1].r, z__3.i = d__1 * csy[i__1].i;
	z_div(&z__2, &z__3, z__);
	z__1.r = csy[i__4].r - z__2.r, z__1.i = csy[i__4].i - z__2.i;
...
/*       ********************************** */
/* Subroutine */ int ittjyb_(x, ttj, tty)
doublereal *x, *ttj, *tty;
{
    /* Builtin functions */
    double log(), cos(), sin(), sqrt();
...
/*       ********************************** */
/* Subroutine */ int ittjya_(x, ttj, tty)
doublereal *x, *ttj, *tty;
{
    /* System generated locals */
    doublereal d__1, d__2;
...
/*       ********************************** */
/* Subroutine */ int cjylv_(v, z__, cbjv, cdjv, cbyv, cdyv)
doublereal *v;
doublecomplex *z__, *cbjv, *cdjv, *cbyv, *cdyv;
{
    /* System generated locals */
    ...
    /* Builtin functions */
    void z_sqrt(), z_div(), z_log(), pow_zi();
    double pow_di();
    void z_exp();
    integer pow_ii();

    /* Local variables */
    static doublecomplex ceta;
    ...
    extern /* Subroutine */ int cjk_();
    static doublecomplex csj, cfy, cws, csy;
...
/*       ********************************** */
/* Subroutine */ int rmn2l_(m, n, c__, x, df, kd, r2f, r2d, id)
integer *m, *n;
doublereal *c__, *x, *df;
integer *kd;
doublereal *r2f, *r2d;
...
/* L10: */
    ...
    i__1 = nm;
    for (k = 2; k <= i__1; ++k) {
	r__ = r__ * (*m + k - (float)1.) * (*m + k + ip - 1.5) / (k - 1.) / (
		k + ip - 1.5);
	suc += r__ * df[k];
...
L55:
	...
	*id = 10;
    ...
    b0 = *kd * *m / pow_dd(x, &c_b199) / ((float)1. - *kd / (*x * *x)) * *r2f;
    sud = 0.;
    eps2 = 0.;
    i__1 = nm;
    for (k = 1; k <= i__1; ++k) {
...
/*       ********************************** */
/* Subroutine */ int psi_spec__(x, ps)
doublereal *x, *ps;
{
    /* System generated locals */
    integer i__1;
...
/*       ====================================== */
    ...
    el = .5772156649015329;
    s = 0.;
    if (*x == (doublereal) ((integer) (*x)) && *x <= (float)0.) {
	*ps = 1e300;
	return 0;
...
/* L10: */
	...
	*ps = -el + s;
    } else if (xa + (float).5 == (doublereal) ((integer) (xa + (float).5))) {
	n = (integer) (xa - (float).5);
	i__1 = n;
	for (k = 1; k <= i__1; ++k) {
/* L20: */
	    s += (float)1. / (k * 2. - 1.);
...
doublereal *q, *a;
{
    ...
    static doublereal delta, a1, a2, q1, q2;
    static integer nn;
    static doublereal qq;
    extern /* Subroutine */ int refine_(), cv0_();
...
/*       ====================================================== */
    ...
    if (*m <= 12 || *q <= *m * (float)3. || *q > (doublereal) (*m * *m)) {
	cv0_(kd, m, q, a);
	if (*q != 0. && *m != 2) {
	    refine_(kd, m, q, a);
	}
	...
	ndiv = 10;
	delta = (*m - (float)3.) * *m / ndiv;
	if (*q - *m * (float)3. <= *m * *m - *q) {
L5:
	    nn = (integer) ((*q - *m * (float)3.) / delta) + 1;
	    delta = (*q - *m * (float)3.) / nn;
	    q1 = *m * (float)2.;
	    cvqm_(m, &q1, &a1);
	    q2 = *m * (float)3.;
	    cvqm_(m, &q2, &a2);
...
/*       ********************************** */
/* Subroutine */ int lpmns_(m, n, x, pm, pd)
integer *m, *n;
doublereal *x, *pm, *pd;
{
    /* System generated locals */
...
/* L10: */
	...
	for (k = 0; k <= i__1; ++k) {
		...
		pd[k] = (k + (float)2.) * -.25 * (k + (float)1.) * k * (k - (
			float)1.);
		if (*x < (float)0.) {
		    i__2 = k + 1;
		    pd[k] = pow_ii(&c_n1, &i__2) * pd[k];
...
/* L25: */
    ...
    pd[0] = ((1. - *m) * pm[1] - *x * pm[0]) / (*x * *x - (float)1.);
    i__1 = *n;
    for (k = 1; k <= i__1; ++k) {
/* L30: */
...
/*       ********************************** */
/* Subroutine */ int cerf_(z__, cer, cder)
doublecomplex *z__, *cer, *cder;
{
    /* System generated locals */
    doublereal d__1;
    doublecomplex z__1, z__2, z__3, z__4;
    ...
    /* Builtin functions */
    double d_imag(), sqrt(), exp(), cos(), sin(), cosh(), sinh();
    void z_exp();
...
/*       ********************************** */
/* Subroutine */ int rswfp_(m, n, c__, x, cv, kf, r1f, r1d, r2f, r2d)
integer *m, *n;
doublereal *c__, *x, *cv;
integer *kf;
doublereal *r1f, *r1d, *r2f, *r2d;
{
    extern /* Subroutine */ int sdmn_(), rmn2l_();
    static doublereal df[200];
    static integer id, kd;
    extern /* Subroutine */ int rmn2sp_(), rmn1_();
...
/*       ********************************** */
/* Subroutine */ int jyndd_(n, x, bjn, djn, fjn, byn, dyn, fyn)
integer *n;
doublereal *x, *bjn, *djn, *fjn, *byn, *dyn, *fyn;
{
    /* System generated locals */
    integer i__1;

    /* Local variables */
    extern /* Subroutine */ int jynbh_();
...
/*       ============================================= */
	...
	*ci = ((((x2 * -3e-8 + 3.1e-6) * x2 - 2.3148e-4) * x2 + .01041667) * 
		x2 - (float).25) * x2 + .577215665 + log(*x);
...
/*       ********************************** */
/* Subroutine */ int refine_(kd, m, q, a)
integer *kd, *m;
doublereal *q, *a;
{
    /* System generated locals */
    ...
    /* Local variables */
    static doublereal f, x, delta, f0, f1, x0, x1, ca;
    static integer mj, it;
    extern /* Subroutine */ int cvf_();
    static doublereal eps;
...
/*       ======================================================== */
	...
	x = x1 - (x1 - x0) / (1. - f0 / f1);
	cvf_(kd, m, q, &x, &mj, &f);
	if ((d__1 = (float)1. - x1 / x, abs(d__1)) < eps || f == (float)0.) {
	    goto L15;
	}
	x0 = x1;
	f0 = f1;
...
/* L20: */
	...
	m = (integer) (*x * (float).82 + (float)47.2);
	xa1 = 0.;
	xa0 = 1e-100;
	for (k = m; k >= 1; --k) {
	    xa = k * 4. * xa0 / *x - xa1;
...
/* L35: */
	...
	for (k = 2; k <= i__1; ++k) {
/* Computing 2nd power */
	    r__1 = k * (float)2. - (float)3.;
/* Computing 2nd power */
	    r__2 = k * (float)2. - (float)1.;
	    xr = xr * .25 * (r__1 * r__1) / ((k - (float)1.) * (r__2 * r__2)) 
		    * *x;
/* L40: */
	    xg1 += bj[k - 1] * xr;
	}
	...
	for (k = 2; k <= i__1; ++k) {
/* Computing 2nd power */
	    r__1 = k * (float)2. - (float)5.;
/* Computing 2nd power */
	    r__2 = k * (float)2. - (float)3.;
	    xr = xr * .25 * (r__1 * r__1) / ((k - (float)1.) * (r__2 * r__2)) 
		    * *x;
/* L45: */
	    xg2 += bj[k - 1] * xr;
...
doublereal *x, *sl1;
{
    ...
    /* Local variables */
    static integer k;
    static doublereal r__, s, a1;
    static integer km;
    static doublereal pi, bi1;
...
L15:
	*sl1 = 2. / pi * s;
	...
	s = 1.;
	km = (integer) (*x * (float).5);
	if (*x > 50.) {
	    km = 25;
	}
	i__1 = km;
	for (k = 1; k <= i__1; ++k) {
	    r__ = r__ * (k * 2. + 3.) * (k * 2. + 1.) / (*x * *x);
	    s += r__;
	    if ((d__1 = r__ / s, abs(d__1)) < 1e-12) {
		goto L25;
	    }
...
/* Computing 4th power */
	d__1 = *x, d__1 *= d__1;
	*sl1 = 2. / pi * (1. / (*x * *x) - 1. + s * 3. / (d__1 * d__1));
	a1 = exp(*x) / sqrt(pi * 2. * *x);
	r__ = 1.;
	bi1 = 1.;
	for (k = 1; k <= 16; ++k) {
/* Computing 2nd power */
	    d__1 = k * 2. - 1.;
	    r__ = r__ * -.125 * (4. - d__1 * d__1) / (k * *x);
	    bi1 += r__;
	    if ((d__1 = r__ / bi1, abs(d__1)) < 1e-12) {
		goto L35;
	    }
/* L30: */
	}
...
/*       ********************************** */
/* Subroutine */ int clqn_(n, x, y, cqn, cqd)
integer *n;
doublereal *x, *y;
doublecomplex *cqn, *cqd;
{
    /* System generated locals */
    integer i__1, i__2, i__3, i__4;
    doublereal d__1, d__2, d__3;
...
/* L15: */
	...
	if (z_abs(&z__) > 1.1) {
	    ...
	    z__1.r = z__.r - (float)1., z__1.i = z__.i;
	    km = (*n + 40) * (integer) ((float)-1. - log(z_abs(&z__1)) * (
		    float)1.8);
	}
	cqf2.r = 0., cqf2.i = 0.;
	cqf1.r = 1., cqf1.i = 0.;
...
doublereal *x, *sl0;
{
    ...
    /* Local variables */
    static integer k;
    static doublereal r__, s, a0, a1;
    static integer km;
    static doublereal pi, bi0;
...
/* Computing 2nd power */
	    d__1 = k * 2. - 1.;
	    r__ = r__ * .125 * (d__1 * d__1) / (k * *x);
	    bi0 += r__;
	    if ((d__1 = r__ / bi0, abs(d__1)) < 1e-12) {
		goto L35;
	    }
/* L30: */
	}
...
/*       ********************************** */
/* Subroutine */ int airyzo_(nt, kf, xa, xb, xc, xd)
integer *nt, *kf;
doublereal *xa, *xb, *xc, *xd;
{
    ...
    extern /* Subroutine */ int airyb_();
    static doublereal u1, ad, bd, ai, bi, pi, rt, rt0, err;
...
/*             DLMF 9.9.18 */
	    d__1 = u * u;
	    rt0 = -pow_dd(&d__1, &c_b285) * (u1 * (u1 * (u1 * (u1 * 
...
/*       ********************************** */
/* Subroutine */ int cerror_(z__, cer)
doublecomplex *z__, *cer;
{
    /* System generated locals */
    doublereal d__1;
    doublecomplex z__1, z__2, z__3, z__4;
...
/*       ********************************** */
/* Subroutine */ int cva1_(kd, m, q, cv)
integer *kd, *m;
doublereal *q, *cv;
{
    /* System generated locals */
...
/* L20: */
	...
	nm = (integer) (*m * (float)1.5 + 10 + *q * (float).5);
	e[0] = 0.;
	f[0] = 0.;
	if (*kd == 1) {
	    d__[0] = 0.;
...
/*       ********************************** */
/* Subroutine */ int ittikb_(x, tti, ttk)
doublereal *x, *tti, *ttk;
{
    /* Builtin functions */
    double exp(), sqrt(), log();
...
/*       ********************************** */
/* Subroutine */ int lqnb_(n, x, qn, qd)
integer *n;
doublereal *x, *qn, *qd;
{
    /* System generated locals */
...
/* L10: */
	...
	for (j = 1; j <= i__2; ++j) {
	    l3 = k * (k + 1) / 2 + j + 1;
	    l4 = (k + 1) * (k + 2) / 2 + j + 1;
	    a[l4] = (j + k * .5 + .125 / (j * (float)2. + k + (float)1.)) * a[
		    l3] - (j + k * .5 - (float)1. + .625 / (j * (float)2. + k 
		    + (float)1.)) * a[l3 - 1];
/* L15: */
	}
    }
...
/*       ********************************** */
/* Subroutine */ int ittika_(x, tti, ttk)
doublereal *x, *tti, *ttk;
{
    /* Initialized data */
...
/*       ********************************** */
/* Subroutine */ int lamv_(v, x, vm, vl, dl)
doublereal *v, *x, *vm, *vl, *dl;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1, d__2;
    ...
    static doublereal r0, v0, x2, ga, bk, ck, rc, cs, pi, sk, uk, vk, rp, rq, 
	    xk, px, qx, vv, elsecs, rp2, fac;
    extern /* Subroutine */ int gam0_();
...
L25:
	...
	i__1 = k0;
	for (k = 1; k <= i__1; ++k) {
	    d__1 = (doublereal) (k * (float)4. - (float)3.);
	    d__2 = (doublereal) (k * (float)4. - (float)1.);
	    rp = rp * -.0078125 * (vv - pow_dd(&d__1, &c_b4)) * (vv - pow_dd(&
		    d__2, &c_b4)) / (k * (k * (float)2. - (float)1.) * x2);
/* L30: */
	    px += rp;
	}
	qx = 1.;
	...
	i__1 = k0;
	for (k = 1; k <= i__1; ++k) {
	    d__1 = (doublereal) (k * (float)4. - (float)1.);
	    d__2 = (doublereal) (k * (float)4. + (float)1.);
...
/*       ********************************** */
/* Subroutine */ int chguit_(a, b, x, hu, id)
doublereal *a, *b, *x, *hu;
integer *id;
{
    /* Initialized data */
    ...
    static integer j, k, m;
    static doublereal s, a1, b1, f1, f2;
    extern /* Subroutine */ int gamma2_();
    static doublereal t1, t2, t3, t4, ga, hu0, hu1, hu2;
...
/*       ********************************** */
/* Subroutine */ int kmn_(m, n, c__, cv, kd, df, dn, ck1, ck2)
integer *m, *n;
doublereal *c__, *cv;
integer *kd;
doublereal *df, *dn, *ck1, *ck2;
{
    /* System generated locals */
    integer i__1, i__2;
    doublereal d__1;
...
/* L30: */
    ...
    dn[*m + 1] = pow_ii(&c_n1, &ip) * dnp * cs / ((*m * (float)2. - (float)1.)
	     * (*m * (float)2. + (float)1. - ip * (float)4.) * tp[*m]);
    i__1 = nn;
    for (k = *m + 2; k <= i__1; ++k) {
/* L35: */
	dn[k] = rk[k - 1] * dn[k - 1];
...
/* L65: */
    ...
    sa0 = ((*m + ip) * (float)2. + (float)1.) * r1 / (pow_ri(&c_b381, n) * 
	    pow_di(c__, &ip) * r2 * r3 * df[1]);
    *ck1 = sa0 * su0;
    if (*kd == -1) {
	return 0;
    }
...
/* L80: */
    ...
    i__1 = ip + 1;
    sb0 = (ip + (float)1.) * pow_di(c__, &i__1) / (ip * (float)2. * (*m - (
	    float)2.) + (float)1.) / (*m * (float)2. - (float)1.);
    *ck2 = pow_ii(&c_n1, &ip) * sb0 * r4 * r5 * g0 / r1 * su0;
    return 0;
} /* kmn_ */

/*       ********************************** */
/* Subroutine */ int lagzo_(n, x, w)
integer *n;
doublereal *x, *w;
{
    /* System generated locals */
...
/*       ========================================================= */
	...
	if (nr > 1) {
	    d__1 = (doublereal) nr;
	    z__ = x[nr - 1] + hn * pow_dd(&d__1, &c_b387);
	}
	it = 0;
L10:
	++it;
...
doublereal *va, *x, *pv;
{
    ...
    /* Builtin functions */
    double exp(), sqrt(), pow_dd(), sin(), cos();

    /* Local variables */
    extern /* Subroutine */ int dvla_();
    static integer k;
    static doublereal r__, a0;
    extern /* Subroutine */ int gamma2_();
    static doublereal x1, gl, qe, pi, pdl, dsl, eps;
...
/*       =================================================== */
    ...
    *pv = 1.;
    for (k = 1; k <= 18; ++k) {
	r__ = r__ * .5 * (k * (float)2. + *va - (float)1.) * (k * (float)2. + 
		*va) / (k * *x * *x);
	*pv += r__;
...
/*       ********************************** */
/* Subroutine */ int cjyva_(v, z__, vm, cbj, cdj, cby, cdy)
doublereal *v;
doublecomplex *z__;
doublereal *vm;
doublecomplex *cbj, *cdj, *cby, *cdy;
{
    /* System generated locals */
    integer i__1, i__2, i__3, i__4;
    real r__1;
    ...
    void pow_zz();
    double pow_dd();
    void z_div(), z_sqrt(), z_cos(), z_sin();
    double cos(), sin();
    void z_log(), z_exp();
    double d_imag();

    /* Local variables */
    static doublecomplex cjvl, cylk, cfac0, cfac1;
    extern integer msta1_(), msta2_();
    static integer j, k, l, n, m;
    static doublereal a0;
    static integer k0;
    extern /* Subroutine */ int gamma2_();
...
/* L25: */
	...
	for (j = 0; j <= 1; ++j) {
		...
		d__2 = (doublereal) (k * (float)4. - (float)3.);
		d__1 = vv - pow_dd(&d__2, &c_b4);
		z__3.r = d__1 * z__4.r, z__3.i = d__1 * z__4.i;
		d__4 = (doublereal) (k * (float)4. - (float)1.);
		d__3 = vv - pow_dd(&d__4, &c_b4);
		z__2.r = d__3 * z__3.r, z__2.i = d__3 * z__3.i;
		r__1 = k * (k * (float)2. - (float)1.);
...
/*       ********************************** */
/* Subroutine */ int cjyvb_(v, z__, vm, cbj, cdj, cby, cdy)
doublereal *v;
doublecomplex *z__;
doublereal *vm;
doublecomplex *cbj, *cdj, *cby, *cdy;
{
    ...
    /* Local variables */
    static doublecomplex cjvn, cfac0;
    extern integer msta1_(), msta2_();
...
/*       ********************************** */
/* Subroutine */ int jy01a_(x, bj0, dj0, bj1, dj1, by0, dy0, by1, dy1)
doublereal *x, *bj0, *dj0, *bj1, *dj1, *by0, *dy0, *by1, *dy1;
{
    /* Initialized data */
    ...
    /* Builtin functions */
    double log(), pow_di(), sqrt(), cos(), sin();
...
/*       ********************************** */
/* Subroutine */ int incog_(a, x, gin, gim, gip)
doublereal *a, *x, *gin, *gim, *gip;
{
    /* System generated locals */
    doublereal d__1;
...
/*       =================================================== */
    ...
    xam = -(*x) + *a * log(*x);
    if (xam > (float)700. || *a > (float)170.) {
	s_wsle(&io___797);
	do_lio(&c__9, &c__1, "a and/or x too large", (ftnlen)20);
	e_wsle();
	s_stop("", (ftnlen)0);
    }
    if (*x == (float)0.) {
...
/*       ********************************** */
/* Subroutine */ int jyv_(v, x, vm, bj, dj, by, dy)
doublereal *v, *x, *vm, *bj, *dj, *by, *dy;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1, d__2;
    ...
    /* Builtin functions */
    double pow_dd(), sqrt(), cos(), sin(), log();

    /* Local variables */
    static doublereal bjvl, byvk;
    extern integer msta1_(), msta2_();
    static doublereal a, b, f;
    static integer j, k, l, m, n;
    static doublereal r__, a0, f0, f1, f2;
    static integer k0;
    static doublereal r0, r1;
    extern /* Subroutine */ int gamma2_();
    static doublereal v0, w0, w1, x2, ga, gb, ec, ck, el, cs, pi, vg, sk, vl, 
	    rp, rq, xk, px, qx, vv, cs0, cs1, rp2, pv0, pv1, bju0, bjv0, bjv1,
	     bju1, byv0, byv1;
...
/* L25: */
	...
	for (j = 0; j <= 1; ++j) {
		...
		rp = rp * -.0078125 * (vv - pow_dd(&d__1, &c_b4)) * (vv - 
			pow_dd(&d__2, &c_b4)) / (k * (k * (float)2. - (float)
			1.) * x2);
/* L30: */
		px += rp;
	    }
...
/*       ********************************** */
/* Subroutine */ int jynb_(n, x, nm, bj, dj, by, dy)
integer *n;
doublereal *x;
integer *nm;
doublereal *bj, *dj, *by, *dy;
{
    /* System generated locals */
    integer i__1;
...
/*       ********************************** */
/* Subroutine */ int jynbh_(n, nmin, x, nm, bj, by)
integer *n, *nmin;
doublereal *x;
integer *nm;
doublereal *bj, *by;
...
/* L10: */
    ...
    if (*x <= (float)300. || *n > (integer) (*x * (float).9)) {
...
doublereal *x, *sh1;
{
    ...
    /* Local variables */
    static integer k;
    static doublereal r__, s, t, a0, p1, q1, t2;
    static integer km;
    static doublereal pi, ta1, by1;
...
L25:
	t = 4. / *x;
	t2 = t * t;
	p1 = ((((t2 * 4.2414e-6 - 2.0092e-5) * t2 + 5.80759e-5) * t2 - 
		2.23203e-4) * t2 + .0029218256) * t2 + .3989422819;
	q1 = t * (((((t2 * -3.6594e-6 + 1.622e-5) * t2 - 3.98708e-5) * t2 + 
		1.064741e-4) * t2 - 6.3904e-4) * t2 + .0374008364);
	ta1 = *x - pi * .75;
	by1 = 2. / sqrt(*x) * (p1 * sin(ta1) + q1 * cos(ta1));
	*sh1 = (float)2. / pi * (s / (*x * *x) + 1.) + by1;
    }
    return 0;
} /* stvh1_ */

/*       ********************************** */
/* Subroutine */ int legzo_(n, x, w)
integer *n;
doublereal *x, *w;
{
    /* System generated locals */
...
/*       ********************************** */
/* Subroutine */ int aswfa_(m, n, c__, x, kd, cv, s1f, s1d)
integer *m, *n;
doublereal *c__, *x;
integer *kd;
doublereal *cv, *s1f, *s1d;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1;
    ...
    /* Local variables */
    extern /* Subroutine */ int sckb_(), sdmn_();
    static integer k;
...
/*       ********************************** */
/* Subroutine */ int jyna_(n, x, nm, bj, dj, by, dy)
integer *n;
doublereal *x;
integer *nm;
doublereal *bj, *dj, *by, *dy;
...
/*       ********************************** */
/* Subroutine */ int pbdv_(v, x, dv, dp, pdf, pdd)
doublereal *v, *x, *dv, *dp, *pdf, *pdd;
{
    ...
    /* Local variables */
    extern /* Subroutine */ int dvla_(), dvsa_();
    static doublereal f;
...
/*       =============================================================== */
	...
	pu = sqrt(pi * (nr * 4. - .5));
	pv = pi * sqrt(nr * 2. - .25);
	px = pu * (float).5 - log(pv) * (float).5 / pu;
	py = pu * (float).5 + log(pv) * (float).5 / pu;
	z__1.r = px, z__1.i = py;
	z__.r = z__1.r, z__.i = z__1.i;
	it = 0;
L15:
...
/*       ********************************** */
/* Subroutine */ int chgu_(a, b, x, hu, md)
doublereal *a, *b, *x, *hu;
integer *md;
{
    /* System generated locals */
    ...
    /* Local variables */
    extern /* Subroutine */ int chgul_(), chgus_();
    static doublereal a00, aa, b00;
    static integer id;
    static logical bn;
    extern /* Subroutine */ int chgubi_(), chguit_();
    static integer id1;
...
/*       ======================================================= */
    ...
    aa = *a - *b + 1.;
    il1 = *a == (doublereal) ((integer) (*a)) && *a <= (float)0.;
    il2 = aa == (doublereal) ((integer) aa) && aa <= (float)0.;
    il3 = (d__1 = *a * (*a - *b + (float)1.), abs(d__1)) / *x <= (float)2.;
    bl1 = *x <= (float)5. || *x <= (float)10. && *a <= (float)2.;
    bl2 = *x > (float)5. && *x <= (float)12.5 && (*a >= (float)1. && *b >= *a 
	    + (float)4.);
    bl3 = *x > (float)12.5 && *a >= (float)5. && *b >= *a + (float)5.;
    bn = *b == (doublereal) ((integer) (*b)) && *b != (float)0.;
    id1 = -100;
    hu1 = 0.;
	...
	*md = 2;
    ...
    if (id < 6) {
	s_wsle(&io___1033);
	do_lio(&c__9, &c__1, "No accurate result obtained", (ftnlen)27);
	e_wsle();
    }
    return 0;
} /* chgu_ */
...
/*       ********************************** */
/* Subroutine */ int lamn_(n, x, nm, bl, dl)
integer *n;
doublereal *x;
integer *nm;
doublereal *bl, *dl;
...
/*       ********************************** */
/* Subroutine */ int comelp_(hk, ck, ce)
doublereal *hk, *ck, *ce;
{
    /* Builtin functions */
    double log();
...
/*       ********************************** */
/* Subroutine */ int incob_(a, b, x, bix)
doublereal *a, *b, *x, *bix;
{
    /* System generated locals */
    doublereal d__1;
...
/* L20: */
	...
	*bix = pow_dd(x, a) * pow_dd(&d__1, b) / (*a * bt) * ta;
	...
	for (k = 1; k <= 20; ++k) {
/* L25: */
	    fk[(k << 1) - 1] = k * (*a - k) * (1. - *x) / (*b + k * (float)2. 
		    - (float)1.) / (*b + k * (float)2.);
	}
	for (k = 0; k <= 20; ++k) {
/* L30: */
...
/*       ********************************** */
/* Subroutine */ int cvf_(kd, m, q, a, mj, f)
integer *kd, *m;
doublereal *q, *a;
integer *mj;
doublereal *f;
...
/*       ********************************** */
/* Subroutine */ int clpn_(n, x, y, cpn, cpd)
integer *n;
doublereal *x, *y;
doublecomplex *cpn, *cpd;
{
    /* System generated locals */
    integer i__1, i__2, i__3;
    doublereal d__1, d__2;
...
/*       ********************************** */
/* Subroutine */ int lqmns_(m, n, x, qm, qd)
integer *m, *n;
doublereal *x, *qm, *qd;
{
    /* System generated locals */
...
/* L15: */
    ...
    xq = sqrt(ls * (1. - *x * *x));
    q0 = log((d__1 = (*x + (float)1.) / (*x - (float)1.), abs(d__1))) * .5;
    q00 = q0;
    q10 = -1. / xq;
    q01 = *x * q0 - 1.;
    ...
    i__1 = *m;
    for (k = 2; k <= i__1; ++k) {
	qm0 = (k - (float)1.) * -2. / xq * *x * qf1 - ls * (k - (float)1.) * (
		(float)2. - k) * qf0;
	qf0 = qf1;
/* L20: */
	qf1 = qm0;
	...
	if (*m == 0 && *n > 0) {
	    ...
	    i__1 = *n;
	    for (k = 2; k <= i__1; ++k) {
		qf2 = ((k * (float)2. - 1.) * *x * qf1 - (k - (float)1.) * 
			qf0) / k;
		qm[k] = qf2;
		qf0 = qf1;
/* L25: */
	...
	i__1 = *m;
	for (k = 2; k <= i__1; ++k) {
	    qm1 = (k - (float)1.) * -2. / xq * *x * qg1 - ls * k * ((float)3. 
		    - k) * qg0;
	    qg0 = qg1;
/* L30: */
	    qg1 = qm1;
...
/* L35: */
		...
		q1l = ((l * (float)2. - 1.) * *x * qh1 - l * qh0) / (l - 1.);
		qf0 = q0l;
		qf1 = q1l;
		i__2 = *m;
		for (k = 2; k <= i__2; ++k) {
		    qmk = (k - (float)1.) * -2. / xq * *x * qf1 - ls * (k + l 
			    - (float)1.) * (l + (float)2. - k) * qf0;
		    qf0 = qf1;
/* L40: */
		    qf1 = qmk;
		}
...
/* L45: */
	...
	qf1 = 1.;
	for (k = km; k >= 0; --k) {
	    qf0 = ((k * (float)2. + 3.) * *x * qf1 - (k + (float)2. - *m) * 
		    qf2) / (k + *m + (float)1.);
	    if (k <= *n) {
		qm[k] = qf0;
	    }
...
/* L60: */
    ...
    qd[0] = ((1. - *m) * qm[1] - *x * qm[0]) / (*x * *x - (float)1.);
    i__1 = *n;
    for (k = 1; k <= i__1; ++k) {
/* L65: */
	qd[k] = (k * *x * qm[k] - (k + *m) * qm[k - 1]) / (*x * *x - (float)
		1.);
    }
    return 0;
} /* lqmns_ */
...
/*       ********************************** */
/* Subroutine */ int ciklv_(v, z__, cbiv, cdiv, cbkv, cdkv)
doublereal *v;
doublecomplex *z__, *cbiv, *cdiv, *cbkv, *cdkv;
{
    /* System generated locals */
    integer i__1, i__2, i__3, i__4, i__5;
    doublereal d__1, d__2;
    ...
    extern /* Subroutine */ int cjk_();
    static doublecomplex csi, csk, cws;
...
/*       ********************************** */
/* Subroutine */ int elit_(hk, phi, fe, ee)
doublereal *hk, *phi, *fe, *ee;
{
    /* Builtin functions */
    double sqrt(), sin(), cos(), log(), tan(), atan();
...
/*       ********************************** */
/* Subroutine */ int elit3_(phi, hk, c__, el3)
doublereal *phi, *hk, *c__, *el3;
{
    /* Initialized data */
...
/*       ========================================================= */
    ...
    lb1 = *hk == 1. && (d__1 = *phi - (float)90., abs(d__1)) <= 1e-8;
    lb2 = *c__ == 1. && (d__1 = *phi - (float)90., abs(d__1)) <= 1e-8;
    if (lb1 || lb2) {
	*el3 = 1e300;
	return 0;
    }
...
/*       ********************************** */
/* Subroutine */ int eixz_(z__, cei)
doublecomplex *z__, *cei;
{
    /* System generated locals */
...
doublereal *x, *sh0;
{
    ...
    /* Local variables */
    static integer k;
    static doublereal r__, s, t, a0, p0, q0, t2;
    static integer km;
    static doublereal pi, ta0, by0;
...
L25:
	t = 4. / *x;
	t2 = t * t;
	p0 = ((((t2 * -3.7043e-6 + 1.73565e-5) * t2 - 4.87613e-5) * t2 + 
		1.7343e-4) * t2 - .001753062) * t2 + .3989422793;
	q0 = t * (((((t2 * 3.2312e-6 - 1.42078e-5) * t2 + 3.42468e-5) * t2 - 
		8.69791e-5) * t2 + 4.564324e-4) * t2 - .0124669441);
	ta0 = *x - pi * .25;
	by0 = 2. / sqrt(*x) * (p0 * sin(ta0) + q0 * cos(ta0));
	*sh0 = 2. / (pi * *x) * s + by0;
    }
    return 0;
} /* stvh0_ */
...
/*       ********************************** */
/* Subroutine */ int hygfx_(a, b, c__, x, hf)
doublereal *a, *b, *c__, *x, *hf;
{
    /* Format strings */
    static char fmt_115[] = "(1x,\002Warning! You should check the accurac\
y\002)";
    ...
    /* Builtin functions */
    integer s_wsle(), do_lio(), e_wsle();
    double sqrt(), pow_dd(), pow_di(), log();
    integer pow_ii(), s_wsfe(), e_wsfe();

    /* Local variables */
    extern /* Subroutine */ int psi_spec__();
    static integer j, k, m;
    static doublereal r__, a0, c0, c1, f0, g0, g1, g2, g3, f1;
    static logical l0, l1, l2, l3, l4, l5;
    static doublereal r0;
    extern /* Subroutine */ int gamma2_();
    static doublereal r1, x1, aa, bb, ga, gb, gc, el, pa, pb, gm, pi;
    static integer nm;
    ...
    static cilist io___1245 = { 0, 6, 0, fmt_115, 0 };
...
/*       ==================================================== */
    ...
    pi = 3.141592653589793;
    el = .5772156649015329;
    l0 = *c__ == (doublereal) ((integer) (*c__)) && *c__ < (float)0.;
    l1 = 1. - *x < 1e-15 && *c__ - *a - *b <= (float)0.;
    l2 = *a == (doublereal) ((integer) (*a)) && *a < (float)0.;
    l3 = *b == (doublereal) ((integer) (*b)) && *b < (float)0.;
    ...
    if (l0 || l1) {
	s_wsle(&io___1206);
	do_lio(&c__9, &c__1, "The hypergeometric series is divergent", (
		ftnlen)38);
	e_wsle();
	return 0;
    }
    ...
    if (*x == (float)0. || *a == (float)0. || *b == (float)0.) {
	*hf = 1.;
	return 0;
    } else if (1. - *x == eps && *c__ - *a - *b > (float)0.) {
	gamma2_(c__, &gc);
	d__1 = *c__ - *a - *b;
	gamma2_(&d__1, &gcab);
	d__1 = *c__ - *a;
	...
	*hf = gc * gcab / (gca * gcb);
	return 0;
    } else if (*x + 1. <= eps && (d__1 = *c__ - *a + *b - (float)1., abs(d__1)
	    ) <= eps) {
	d__1 = -(*a);
...
/* L40: */
		...
		for (k = 1; k <= i__1; ++k) {
/* L45: */
		    sp0 = sp0 + 1. / (*a + k - (float)1.) + (float)1. / (*b + 
			    k - (float)1.) - (float)1. / k;
		}
		f1 = pa + pb + sp0 + el * 2. + log(1. - *x);
		for (k = 1; k <= 250; ++k) {
		    sp = sp + (1. - *a) / (k * (*a + k - (float)1.)) + ((
			    float)1. - *b) / (k * (*b + k - (float)1.));
		    sm = 0.;
		    i__1 = m;
		    for (j = 1; j <= i__1; ++j) {
/* L50: */
			sm = sm + (1. - *a) / ((j + k) * (*a + j + k - (float)
				1.)) + (float)1. / (*b + j + k - (float)1.);
		    }
		    rp = pa + pb + el * 2. + sp + sm + log(1. - *x);
		    r1 = r1 * (*a + m + k - 1.) * (*b + m + k - (float)1.) / (
			    k * (m + k)) * ((float)1. - *x);
		    f1 += r1 * rp;
...
L85:
	    ...
	    *hf = 0.;
	    ...
	    r1 = c1;
	    for (k = 1; k <= 250; ++k) {
		r0 = r0 * (*a + k - 1.) * (*b + k - (float)1.) / (k * (*a + *
			b - *c__ + k)) * ((float)1. - *x);
		r1 = r1 * (*c__ - *a + k - 1.) * (*c__ - *b + k - (float)1.) /
			 (k * (*c__ - *a - *b + k)) * ((float)1. - *x);
		*hf = *hf + r0 + r1;
...
/*       ********************************** */
/* Subroutine */ int cchg_(a, b, z__, chg)
doublereal *a, *b;
doublecomplex *z__, *chg;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1, d__2, d__3;
...
/*       =================================================== */
    ...
    if (*b == (float)0. || *b == (doublereal) (-((integer) abs(*b)))) {
	chg->r = 1e300, chg->i = 0.;
...
/* L20: */
		...
		if (x == (float)0. && y >= (float)0.) {
		    phi = pi * .5;
		} else if (x == (float)0. && y <= (float)0.) {
		    phi = pi * -.5;
		} else {
		    phi = atan(y / x);
		}
		if (phi > pi * (float)-.5 && phi < pi * (float)1.5) {
		    ns = 1;
		}
		if (phi > pi * (float)-1.5 && phi <= pi * (float)-.5) {
		    ns = -1;
		}
		d__1 = (doublereal) ns;
...
/*       ********************************** */
/* Subroutine */ int hygfz_(a, b, c__, z__, zhf)
doublereal *a, *b, *c__;
doublecomplex *z__, *zhf;
{
    /* Format strings */
    static char fmt_160[] = "(1x,\002Warning! You should check the accurac\
y\002)";
    ...
    /* Builtin functions */
    double d_imag(), z_abs(), sqrt(), pow_dd();
    void pow_zz(), z_div();
    double d_sign();
    void pow_zi(), z_log();
    integer pow_ii();
    ...
    /* Fortran I/O blocks */
    static cilist io___1355 = { 0, 6, 0, fmt_160, 0 };
...
L25:
	    ...
	    gm = 0.;
	    d__1 = *c__ - *a - *b;
	    mcab = (integer) (*c__ - *a - *b + eps * d_sign(&c_b597, &d__1));
...
L105:
	...
	d__1 = *a - *b;
	mab = (integer) (*a - *b + eps * d_sign(&c_b597, &d__1));
...
L115:
		...
		*b = aa;
	    ...
	    ca = *c__ - *a;
	    cb = *c__ - *b;
	    nca = (integer) (ca + eps * d_sign(&c_b597, &ca));
	    ncb = (integer) (cb + eps * d_sign(&c_b597, &cb));
...
/*       ********************************** */
/* Subroutine */ int itairy_(x, apt, bpt, ant, bnt)
doublereal *x, *apt, *bpt, *ant, *bnt;
{
    /* Initialized data */
	...
	*bnt = 0.;
	...
	if (abs(*x) <= 9.25) {
		...
		*x = pow_ii(&c_n1, &l) * *x;
		...
		r__ = *x;
		for (k = 1; k <= 40; ++k) {
		    r__ = r__ * (k * (float)3. - 2.) / (k * (float)3. + 1.) * 
			    *x / (k * (float)3.) * *x / (k * (float)3. - 1.) *
			     *x;
		    fx += r__;
...
L15:
		...
		r__ = gx;
		for (k = 1; k <= 40; ++k) {
		    r__ = r__ * (k * (float)3. - 1.) / (k * (float)3. + 2.) * 
			    *x / (k * (float)3.) * *x / (k * (float)3. + 1.) *
			     *x;
		    gx += r__;
...
/*       ********************************** */
/* Subroutine */ int ikna_(n, x, nm, bi, di, bk, dk)
integer *n;
doublereal *x;
integer *nm;
doublereal *bi, *di, *bk, *dk;
{
    /* System generated locals */
    integer i__1;

    /* Local variables */
    extern /* Subroutine */ int ik01a_();
    extern integer msta1_(), msta2_();
...
/* L10: */
    ...
    if (*x > (float)40. && *n < (integer) (*x * (float).25)) {
	h0 = bi0;
	h1 = bi1;
	i__1 = *n;
	for (k = 2; k <= i__1; ++k) {
...
/*       ********************************** */
/* Subroutine */ int cjyna_(n, z__, nm, cbj, cdj, cby, cdy)
integer *n;
doublecomplex *z__;
integer *nm;
doublecomplex *cbj, *cdj, *cby, *cdy;
{
    ...
    /* Local variables */
    extern /* Subroutine */ int cjy01_();
    static doublecomplex cylk;
    extern integer msta1_(), msta2_();
    static integer k, m;
    static doublereal a0;
    static doublecomplex cf;
    static integer lb;
    static doublecomplex cs;
    static doublereal wa, pi;
    static doublecomplex cf1, cf2, cg0, cj0, cj1;
    static integer lb0;
    static doublecomplex cg1, ch2, ch1, ch0;
    static doublereal ya0, ya1;
    static doublecomplex cp11, cp12, cp22, cjk, cp21;
    static doublereal yak;
    static doublecomplex cyk, cbj0, cbj1, cdj0, cdj1, cby0, cby1, cdy0, cdy1, 
	    cyl1, cyl2;
...
/* L5: */
    ...
    cjy01_(z__, &cbj0, &cdj0, &cbj1, &cdj1, &cby0, &cdy0, &cby1, &cdy1);
    cbj[0].r = cbj0.r, cbj[0].i = cbj0.i;
    cbj[1].r = cbj1.r, cbj[1].i = cbj1.i;
    cby[0].r = cby0.r, cby[0].i = cby0.i;
    cby[1].r = cby1.r, cby[1].i = cby1.i;
    cdj[0].r = cdj0.r, cdj[0].i = cdj0.i;
    cdj[1].r = cdj1.r, cdj[1].i = cdj1.i;
    cdy[0].r = cdy0.r, cdy[0].i = cdy0.i;
    cdy[1].r = cdy1.r, cdy[1].i = cdy1.i;
    if (*n <= 1) {
	return 0;
    }
    if (*n < (integer) (a0 * (float).25)) {
	cj0.r = cbj0.r, cj0.i = cbj0.i;
	cj1.r = cbj1.r, cj1.i = cbj1.i;
	i__2 = *n;
	for (k = 2; k <= i__2; ++k) {
	    d__1 = (k - 1.) * 2.;
	    z__4.r = d__1, z__4.i = 0.;
	    z_div(&z__3, &z__4, z__);
	    z__2.r = z__3.r * cj1.r - z__3.i * cj1.i, z__2.i = z__3.r * cj1.i 
		    + z__3.i * cj1.r;
	    z__1.r = z__2.r - cj0.r, z__1.i = z__2.i - cj0.i;
	    cjk.r = z__1.r, cjk.i = z__1.i;
	    i__1 = k;
	    cbj[i__1].r = cjk.r, cbj[i__1].i = cjk.i;
	    cj0.r = cj1.r, cj0.i = cj1.i;
/* L70: */
	    cj1.r = cjk.r, cj1.i = cjk.i;
	}
    } else {
	m = msta1_(&a0, &c__200);
	if (m < *n) {
...
/* L85: */
	...
	z__1.r = cbj[i__2].r - z__2.r, z__1.i = cbj[i__2].i - z__2.i;
	cdj[i__3].r = z__1.r, cdj[i__3].i = z__1.i;
    }
    ya0 = z_abs(&cby0);
    lb = 0;
    lb0 = 0;
    cg0.r = cby0.r, cg0.i = cby0.i;
    cg1.r = cby1.r, cg1.i = cby1.i;
    i__3 = *nm;
    for (k = 2; k <= i__3; ++k) {
	d__1 = (k - 1.) * 2.;
	z__4.r = d__1, z__4.i = 0.;
...
/* L105: */
	...
	i__3 = lb + 1;
	i__2 = lb + 1;
	z__3.r = cbj[i__2].r * cby0.r - cbj[i__2].i * cby0.i, z__3.i = cbj[
		i__2].r * cby0.i + cbj[i__2].i * cby0.r;
	z__5.r = cp11.r * 2., z__5.i = cp11.i * 2.;
	z__6.r = pi * z__->r, z__6.i = pi * z__->i;
	z_div(&z__4, &z__5, &z__6);
	z__2.r = z__3.r - z__4.r, z__2.i = z__3.i - z__4.i;
	...
	i__3 = lb + 1;
	i__2 = lb + 1;
	z__3.r = cbj[i__2].r * cby1.r - cbj[i__2].i * cby1.i, z__3.i = cbj[
		i__2].r * cby1.i + cbj[i__2].i * cby1.r;
	z__5.r = cp21.r * 2., z__5.i = cp21.i * 2.;
	z__6.r = pi * z__->r, z__6.i = pi * z__->i;
	z_div(&z__4, &z__5, &z__6);
	z__2.r = z__3.r - z__4.r, z__2.i = z__3.i - z__4.i;
...
/*       ********************************** */
/* Subroutine */ int cjynb_(n, z__, nm, cbj, cdj, cby, cdy)
integer *n;
doublecomplex *z__;
integer *nm;
doublecomplex *cbj, *cdj, *cby, *cdy;
{
    ...
    void z_div();
    integer pow_ii();
    void z_cos(), z_log(), pow_zi(), z_sqrt(), z_sin();

    /* Local variables */
    static doublecomplex cbjk;
    extern integer msta1_(), msta2_();
    static integer k, m;
    ...
    static doublecomplex cbs, csu, csv, cyy, cbj0, cbj1, cby0, cby1;
...
/*       ********************************** */
/* Subroutine */ int iknb_(n, x, nm, bi, di, bk, dk)
integer *n;
doublereal *x;
integer *nm;
doublereal *bi, *di, *bk, *dk;
...
/*       ********************************** */
/* Subroutine */ int lpmn_(mm, m, n, x, pm, pd)
integer *mm, *m, *n;
doublereal *x, *pm, *pd;
{
    /* System generated locals */
...
/*       ********************************** */
/* Subroutine */ int mtu0_(kf, m, q, x, csf, csd)
integer *kf, *m;
doublereal *q, *x, *csf, *csd;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1;
    ...
    /* Local variables */
    extern doublereal dnan_();
    static doublereal a;
    static integer k;
    extern /* Subroutine */ int fcoef_();
    static integer ic;
...
/*       =============================================================== */
    ...
    cva2_(&kd, m, q, &a);
    if (*q <= 1.) {
	qm = sqrt(*q) * (float)56.1 + (float)7.5 - *q * (float)134.7 + sqrt(*
		q) * (float)90.7 * *q;
    } else {
	qm = sqrt(*q) * (float)3.1 + (float)17. - *q * (float).126 + sqrt(*q) 
		* (float).0037 * *q;
    }
    km = (integer) (qm + *m * (float).5);
    if (km > 251) {
	*csf = dnan_();
	*csd = dnan_();
	return 0;
...
/*       ********************************** */
/* Subroutine */ int cy01_(kf, z__, zf, zd)
integer *kf;
doublecomplex *z__, *zf, *zd;
{
    /* Initialized data */
...
/*       ********************************** */
/* Subroutine */ int ffk_(ks, x, fr, fi, fm, fa, gr, gi, gm, ga)
integer *ks;
doublereal *x, *fr, *fi, *fm, *fa, *gr, *gi, *gm, *ga;
{
    /* System generated locals */
    doublereal d__1;
...
/*       ********************************** */
/* Subroutine */ int airya_(x, ai, bi, ad, bd)
doublereal *x, *ai, *bi, *ad, *bd;
{
    /* Builtin functions */
    double pow_dd(), sqrt();
    ...
    extern /* Subroutine */ int ajyik_();
    static doublereal c1, c2, xa, xq, vi1, vj1, vj2, vi2, vk1, vk2, sr3, vy1, 
	    vy2, pir;
...
/*       ********************************** */
/* Subroutine */ int airyb_(x, ai, bi, ad, bd)
doublereal *x, *ai, *bi, *ad, *bd;
{
    /* System generated locals */
    integer i__1;
...
/*       ********************************** */
/* Subroutine */ int scka_(m, n, c__, cv, kd, ck)
integer *m, *n;
doublereal *c__, *cv;
integer *kd;
doublereal *ck;
...
/* L5: */
	    ...
	    fl = ck[k + 1];
	    f1 = 1.;
	    f2 = ((*m + ip) * (*m + ip + (float)1.) - *cv + cs) * .25 / (*m + 
		    (float)1.) * f1;
	    ck[1] = f1;
	    if (kb == 1) {
		fs = f2;
		...
		ck[2] = f2;
		fs = (((*m + ip + (float)2.) * (*m + ip + (float)3.) - *cv + 
			cs) * f2 - cs * f1) * .125 / (*m + (float)2.);
	    } else {
		ck[2] = f2;
		i__1 = kb + 1;
		for (j = 3; j <= i__1; ++j) {
		    f = (((j * (float)2. + *m + ip - (float)4.) * (j * (float)
			    2. + *m + ip - (float)3.) - *cv + cs) * f2 - cs * 
			    f1) * .25 / ((j - (float)1.) * (j + *m - (float)
			    1.));
		    if (j <= kb) {
			ck[j] = f;
		    }
...
/*       ********************************** */
/* Subroutine */ int sckb_(m, n, c__, df, ck)
integer *m, *n;
doublereal *c__, *df, *ck;
{
    /* System generated locals */
    integer i__1, i__2;
    doublereal d__1;
...
/*       ********************************** */
/* Subroutine */ int cpdla_(n, z__, cdn)
integer *n;
doublecomplex *z__, *cdn;
{
    /* System generated locals */
...
/*       =========================================================== */
	...
	r__1 = k * (float)2. - *n - (float)1.;
	z__3.r = r__1 * z__4.r, z__3.i = r__1 * z__4.i;
	r__2 = k * (float)2. - *n - (float)2.;
...
/*       ********************************** */
/* Subroutine */ int fcszo_(kf, nt, zo)
integer *kf, *nt;
doublecomplex *zo;
{
    /* System generated locals */
    ...
    extern /* Subroutine */ int cfc_(), cfs_();
...
/*       ============================================================== */
	...
	px = psq - log(pi * psq) / (pi * pi * pow_dd(&psq, &c_b199));
	py = log(pi * psq) / (pi * psq);
	z__1.r = px, z__1.i = py;
	z__.r = z__1.r, z__.i = z__1.i;
	if (*kf == 2) {
	    if (nr == 2) {
		z__.r = (float)2.8334, z__.i = (float).2443;
	    }
	    if (nr == 3) {
		z__.r = (float)3.4674, z__.i = (float).2185;
	    }
	    if (nr == 4) {
		z__.r = (float)4.0025, z__.i = (float).2008;
	    }
	}
	it = 0;
L15:
...
/*       ********************************** */
/* Subroutine */ int lpmv0_(v, m, x, pmv)
doublereal *v;
integer *m;
doublereal *x, *pmv;
{
...
/*       ********************************** */
/* Subroutine */ int lpmv_(v, m, x, pmv)
doublereal *v;
integer *m;
doublereal *x, *pmv;
{
    ...
    /* Local variables */
    extern doublereal dinf_(), dnan_();
    extern /* Subroutine */ int lpmv0_();
    static integer j, neg_m__;
    static doublereal g1, g2, p0, p1;
    extern /* Subroutine */ int gamma2_();
    static doublereal v0;
...
/*       ********************************** */
/* Subroutine */ int cgama_(x, y, kf, gr, gi)
doublereal *x, *y;
integer *kf;
doublereal *gr, *gi;
{
    ...
    /* Builtin functions */
    double sqrt(), atan(), log(), pow_di(), cos(), sin(), cosh(), sinh(), exp(
	    );
...
/*       ********************************** */
/* Subroutine */ int aswfb_(m, n, c__, x, kd, cv, s1f, s1d)
integer *m, *n;
doublereal *c__, *x;
integer *kd;
doublereal *cv, *s1f, *s1d;
{
    ...
    /* Local variables */
    extern /* Subroutine */ int sdmn_();
    static integer k;
    extern /* Subroutine */ int lpmns_();
...
/*       ********************************** */
/* Subroutine */ int chgus_(a, b, x, hu, id)
doublereal *a, *b, *x, *hu;
integer *id;
{
    /* System generated locals */
    ...
    /* Builtin functions */
    double sin(), pow_dd(), d_lg10();

    /* Local variables */
    static doublereal hmin, hmax;
    static integer j;
    static doublereal d1, d2, h0, r1;
    extern /* Subroutine */ int gamma2_();
    static doublereal r2, ga, gb, pi, gb2, hu0, xg1, xg2, gab, hua;
...
/* Computing 3rd power */
	    d__1 = k * (float)2. + 1.;
	    r__ = -r__ * *x * *x * (k * (float)2. - 1.) / (d__1 * (d__1 * 
		    d__1));
	    s += r__;
	    if (abs(r__) < abs(s) * 1e-12) {
		goto L15;
...
/* Computing 3rd power */
	    d__1 = k * (float)2. - 1.;
	    r__ = -r__ * (d__1 * (d__1 * d__1)) / ((k * (float)2. + 1.) * *x *
		     *x);
	    s += r__;
	    if (abs(r__) < abs(s) * 1e-12) {
		goto L25;
...
/*       ********************************** */
/* Subroutine */ int lqna_(n, x, qn, qd)
integer *n;
doublereal *x, *qn, *qd;
{
    /* System generated locals */
...
doublereal *va, *x, *pd;
{
    ...
    /* Builtin functions */
    double exp(), pow_dd(), cos();

    /* Local variables */
    extern /* Subroutine */ int vvla_();
    static integer k;
    static doublereal r__, a0;
    extern /* Subroutine */ int gamma2_();
    static doublereal x1, gl, ep, pi, vl, eps;
...
/*       ********************************** */
/* Subroutine */ int ik01a_(x, bi0, di0, bi1, di1, bk0, dk0, bk1, dk1)
doublereal *x, *bi0, *di0, *bi1, *di1, *bk0, *dk0, *bk1, *dk1;
{
    /* Initialized data */
...
/*       ********************************** */
/* Subroutine */ int cpbdn_(n, z__, cpb, cpd)
integer *n;
doublecomplex *z__, *cpb, *cpd;
{
    /* System generated locals */
    integer i__1, i__2, i__3, i__4;
    doublereal d__1;
    ...
    /* Local variables */
    static integer k, m;
    extern /* Subroutine */ int cpdla_();
    static doublereal x;
    extern /* Subroutine */ int cpdsa_();
    static doublereal a0;
...
/* L10: */
	...
	n0 = -(*n);
	if (x <= (float)0. || z_abs(z__) == (float)0.) {
	    cf0.r = ca0.r, cf0.i = ca0.i;
	    cpb[0].r = cf0.r, cpb[0].i = cf0.i;
	    z__1.r = -z__->r, z__1.i = -z__->i;
	    z1.r = z__1.r, z1.i = z__1.i;
...
/*       ********************************** */
/* Subroutine */ int ik01b_(x, bi0, di0, bi1, di1, bk0, dk0, bk1, dk1)
doublereal *x, *bi0, *di0, *bi1, *di1, *bk0, *dk0, *bk1, *dk1;
{
    /* Builtin functions */
    double exp(), sqrt(), log();
...
/*       ********************************** */
/* Subroutine */ int fcoef_(kd, m, q, a, fc)
integer *kd, *m;
doublereal *q, *a, *fc;
{
    /* System generated locals */
...
/*       ********************************** */
/* Subroutine */ int sphi_(n, x, nm, si, di)
integer *n;
doublereal *x;
integer *nm;
doublereal *si, *di;
...
/*       ********************************** */
/* Subroutine */ int pbwa_(a, x, w1f, w1d, w2f, w2d)
doublereal *a, *x, *w1f, *w1d, *w2f, *w2d;
{
    /* System generated locals */
    doublereal d__1;
    ...
    /* Local variables */
    static doublereal d__[100], h__[100];
    extern /* Subroutine */ int cgama_();
    static integer k, m;
...
/* L10: */
	...
	r1 = h__[k - 1] * r__;
	y1f += r1;
	if ((d__1 = r1 / y1f, abs(d__1)) <= eps && k > 30) {
	    goto L20;
	}
/* L15: */
    }
L20:
	...
	r1 = h__[k] * r__;
	y1d += r1;
	if ((d__1 = r1 / y1d, abs(d__1)) <= eps && k > 30) {
	    goto L30;
	}
/* L25: */
    }
...
/* L40: */
	...
	r1 = d__[k] * r__;
	y2f += r1;
	if ((d__1 = r1 / y2f, abs(d__1)) <= eps && k > 30) {
	    goto L50;
	}
/* L45: */
    }
L50:
	...
	r1 = d__[k] * r__;
	y2d += r1;
	if ((d__1 = r1 / y2d, abs(d__1)) <= eps && k > 30) {
	    goto L60;
	}
/* L55: */
    }
...
/*       ********************************** */
/* Subroutine */ int rmn1_(m, n, c__, x, df, kd, r1f, r1d)
integer *m, *n;
doublereal *c__, *x, *df;
integer *kd;
doublereal *r1f, *r1d;
{
    ...
    /* Builtin functions */
    double pow_ri(), pow_di(), pow_dd();

    /* Local variables */
    extern /* Subroutine */ int sckb_(), sphj_();
    static integer j, k, l;
...
L55:
    *r1f *= a0;
    b0 = *kd * *m / pow_dd(x, &c_b199) / ((float)1. - *kd / (*x * *x)) * *r1f;
    sud = 0.;
    sw = 0.;
    i__1 = nm;
    for (k = 1; k <= i__1; ++k) {
...
doublereal *va, *x, *pd;
{
    ...
    /* Builtin functions */
    double sqrt(), exp(), pow_dd();

    /* Local variables */
    static integer m;
    static doublereal r__, a0, g0, g1, r1;
    extern /* Subroutine */ int gamma2_();
    static doublereal ep, gm, pi, vm, vt, ga0, va0, sq2, eps;
...
/*       ********************************** */
/* Subroutine */ int e1z_(z__, ce1)
doublecomplex *z__, *ce1;
{
    /* System generated locals */
    doublereal d__1, d__2, d__3;
    doublecomplex z__1, z__2, z__3, z__4, z__5, z__6, z__7;
...
/*       so use power series in a wedge around it until radius 40.0 */
    ...
    if (a0 == 0.) {
	ce1->r = 1e300, ce1->i = 0.;
    } else if (a0 <= (float)5. || x < xt && a0 < (float)40.) {
/*          Power series */
	ce1->r = 1., ce1->i = 0.;
	cr.r = 1., cr.i = 0.;
	for (k = 1; k <= 500; ++k) {
...
L15:
	if (x <= (float)0. && d_imag(z__) == (float)0.) {
...
/*       ======================================================= */
	...
	*ty = 2. / pi * log(*x / 2.) * *tj - *ty;
	...
	f0 = ((((((t * .001496119 - .00739083) * t + .016236617) * t - 
		...
		.124611058) * 4. / *x;
	g0 = (((((t * .001076103 - .005434851) * t + .01242264) * t - (float)
		.018255209) * t + .023664841) * t - .049635633) * t + 
		.79784879;
...
/*       ********************************** */
/* Subroutine */ int chgul_(a, b, x, hu, id)
doublereal *a, *b, *x, *hu;
integer *id;
{
    /* System generated locals */
...
/*       ********************************** */
/* Subroutine */ int gmn_(m, n, c__, x, bk, gf, gd)
integer *m, *n;
doublereal *c__, *x, *bk, *gf, *gd;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1, d__2;
...
/*       =========================================================== */
    ...
    i__1 = nm;
    for (k = 1; k <= i__1; ++k) {
	d__1 = (doublereal) (k * (float)2. - (float)2.);
...
L15:
    ...
    *gf = xm * gf0 * pow_di(x, &i__1);
	...
	if (ip == 0) {
	    d__1 = (doublereal) (k * (float)2. - (float)2.);
	    gd0 += (k * 2. - (float)1.) * bk[k] * pow_dd(x, &d__1);
	} else {
	    d__1 = (doublereal) (k * (float)2. - (float)1.);
...
doublereal *v, *x, *slv;
{
    ...
    /* Builtin functions */
    integer pow_ii();
    double sqrt(), pow_dd(), exp();

    /* Local variables */
    static integer k, l, n;
    static doublereal r__, s, u, r1;
    extern /* Subroutine */ int gamma2_();
    static doublereal r2, v0, s0, u0, ga, gb, bf, sa, va, vb, pi, vt, bf0, 
	    bf1, biv, biv0;
...
/*       Input :   v   --- Order of Lv(x)  ( |v| ≤ 20 ) */
/*                 x   --- Argument of Lv(x) ( x ≥ 0 ) */
/*       Output:   SLV --- Lv(x) */
/*       Routine called: GAMMA2 to compute the gamma function */
/*       ====================================================== */

    pi = 3.141592653589793;
    if (*x == 0.) {
	if (*v > (float)-1. || (integer) (*v) - *v == .5) {
	    *slv = 0.;
	} else if (*v < -1.) {
	    i__1 = (integer) (.5 - *v) - 1;
	    *slv = pow_ii(&c_n1, &i__1) * 1e300;
	} else if (*v == -1.) {
	    *slv = 2. / pi;
	}
	return 0;
...
L15:
	d__1 = *x * .5;
	d__2 = *v + 1.;
	*slv = pow_dd(&d__1, &d__2) * s;
    } else {
	d__1 = *x * .5;
	d__2 = *v - (float)1.;
	sa = -1. / pi * pow_dd(&d__1, &d__2);
	v0 = *v + .5;
	gamma2_(&v0, &ga);
	s = -sqrt(pi) / ga;
	r1 = -1.;
...
/* Computing 2nd power */
		d__1 = k * (float)2. - 1.;
		r__ = r__ * (float)-.125 * (vt * (float)4. * vt - d__1 * d__1)
			 / (k * *x);
		biv += r__;
		if ((d__1 = r__ / biv, abs(d__1)) < 1e-12) {
		    goto L30;
		}
/* L25: */
	    }
...
/* L40: */
	...
	*slv = exp(*x) / sqrt(pi * 2. * *x) * biv + s0;
    }
    return 0;
} /* stvlv_ */

/*       ********************************** */
/* Subroutine */ int rcty_(n, x, nm, ry, dy)
integer *n;
doublereal *x;
integer *nm;
doublereal *ry, *dy;
...
/*       ********************************** */
/* Subroutine */ int lpni_(n, x, pn, pd, pl)
integer *n;
doublereal *x, *pn, *pd, *pl;
{
    /* System generated locals */
    integer i__1, i__2;
...
/*       ********************************** */
/* Subroutine */ int klvna_(x, ber, bei, ger, gei, der, dei, her, hei)
doublereal *x, *ber, *bei, *ger, *gei, *der, *dei, *her, *hei;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1;
...
L65:
	r__ = *x * -.25 * x2;
	gs = 1.5;
	*her = r__ * 1.5 - *ber / *x - (log(*x / 2.) + el) * *der + pi * (
		float).25 * *dei;
	for (m = 1; m <= 60; ++m) {
/* Computing 2nd power */
	    d__1 = m * 2. + 1.;
...
L75:
	r__ = *x * .5;
	gs = 1.;
	*hei = *x * .5 - *bei / *x - (log(*x / 2.) + el) * *dei - pi * (float)
		.25 * *der;
	for (m = 1; m <= 60; ++m) {
...
/*       ********************************** */
/* Subroutine */ int chgubi_(a, b, x, hu, id)
doublereal *a, *b, *x, *hu;
integer *id;
{
    /* System generated locals */
    ...
    /* Builtin functions */
    integer pow_ii();
    double pow_di(), d_lg10(), log();

    /* Local variables */
    static doublereal hmin, hmax;
    extern /* Subroutine */ int psi_spec__();
    static integer j, k, m, n;
    static doublereal r__, a0, a1, a2, h0, s0;
    extern /* Subroutine */ int gamma2_();
    static doublereal s1, s2, ga, el, sa, sb, ua, ub, hw, rn, ps, da1, da2, 
	    db1, ga1, db2;
    static integer id1, id2;
    static doublereal hm1, hm2, hm3, hu1, hu2, rn1;
...
/*       ********************************** */
/* Subroutine */ int cyzo_(nt, kf, kc, zo, zv)
integer *nt, *kf, *kc;
doublecomplex *zo, *zv;
{
    /* System generated locals */
...
/*       ********************************** */
/* Subroutine */ int klvnb_(x, ber, bei, ger, gei, der, dei, her, hei)
doublereal *x, *ber, *bei, *ger, *gei, *der, *dei, *her, *hei;
{
    /* Builtin functions */
    double log();
...
/*       ********************************** */
/* Subroutine */ int rmn2so_(m, n, c__, x, cv, df, kd, r2f, r2d)
integer *m, *n;
doublereal *c__, *x, *cv, *df;
integer *kd;
doublereal *r2f, *r2d;
{
    ...
    /* Local variables */
    extern /* Subroutine */ int sckb_();
    static integer j;
    static doublereal h0;
    extern /* Subroutine */ int qstar_();
    static doublereal gd, bk[200], ck[200], gf, dn[200], pi;
    static integer nm, ip;
    static doublereal qs, qt, sw, ck1, ck2, r1d, r1f;
    extern /* Subroutine */ int cbk_(), gmn_(), kmn_();
    static doublereal eps, sum;
    extern /* Subroutine */ int rmn1_();
...
/*       ********************************** */
/* Subroutine */ int csphik_(n, z__, nm, csi, cdi, csk, cdk)
integer *n;
doublecomplex *z__;
integer *nm;
doublecomplex *csi, *cdi, *csk, *cdk;
{
    /* System generated locals */
    integer i__1, i__2, i__3, i__4;
    doublereal d__1;
    ...
    /* Builtin functions */
    double z_abs();
    void z_sin(), z_div(), z_cos(), z_exp(), pow_zi();

    /* Local variables */
    extern integer msta1_(), msta2_();
    static integer k, m;
    static doublecomplex ccosh, csinh;
    static doublereal a0;
    static doublecomplex cf, ci, cs;
    static doublereal pi;
    static doublecomplex cf0, cf1, csi0, csi1;
...
/*       ======================================================= */
    ...
    *nm = *n;
	...
	for (k = 0; k <= i__1; ++k) {
	    i__2 = k;
	    csi[i__2].r = 0., csi[i__2].i = 0.;
	    i__2 = k;
	    cdi[i__2].r = 0., cdi[i__2].i = 0.;
	    i__2 = k;
	    csk[i__2].r = 1e300, csk[i__2].i = 0.;
/* L10: */
	    i__2 = k;
	    cdk[i__2].r = -1e300, cdk[i__2].i = 0.;
	}
	csi[0].r = 1., csi[0].i = 0.;
	cdi[1].r = .3333333333333333, cdi[1].i = 0.;
	return 0;
    }
    ci.r = 0., ci.i = 1.;
    z__3.r = ci.r * z__->r - ci.i * z__->i, z__3.i = ci.r * z__->i + ci.i * 
	    z__->r;
    z_sin(&z__2, &z__3);
    z_div(&z__1, &z__2, &ci);
    csinh.r = z__1.r, csinh.i = z__1.i;
    z__2.r = ci.r * z__->r - ci.i * z__->i, z__2.i = ci.r * z__->i + ci.i * 
	    z__->r;
    z_cos(&z__1, &z__2);
    ccosh.r = z__1.r, ccosh.i = z__1.i;
    z_div(&z__1, &csinh, z__);
    csi0.r = z__1.r, csi0.i = z__1.i;
    z__4.r = -csinh.r, z__4.i = -csinh.i;
    z_div(&z__3, &z__4, z__);
    z__2.r = z__3.r + ccosh.r, z__2.i = z__3.i + ccosh.i;
    z_div(&z__1, &z__2, z__);
    csi1.r = z__1.r, csi1.i = z__1.i;
    csi[0].r = csi0.r, csi[0].i = csi0.i;
    csi[1].r = csi1.r, csi[1].i = csi1.i;
    if (*n >= 2) {
	m = msta1_(&a0, &c__200);
	if (m < *n) {
	    *nm = m;
	...
	for (k = m; k >= 0; --k) {
	    ...
	    if (k <= *nm) {
		i__2 = k;
		csi[i__2].r = cf.r, csi[i__2].i = cf.i;
	    }
	    cf0.r = cf1.r, cf0.i = cf1.i;
/* L15: */
	    cf1.r = cf.r, cf1.i = cf.i;
	}
	if (z_abs(&csi0) > z_abs(&csi1)) {
	    z_div(&z__1, &csi0, &cf);
	    cs.r = z__1.r, cs.i = z__1.i;
	}
	if (z_abs(&csi0) <= z_abs(&csi1)) {
...
/* L20: */
	    i__1 = k;
	    i__3 = k;
	    z__1.r = cs.r * csi[i__3].r - cs.i * csi[i__3].i, z__1.i = cs.r * 
		    csi[i__3].i + cs.i * csi[i__3].r;
	    csi[i__1].r = z__1.r, csi[i__1].i = z__1.i;
	}
    }
    cdi[0].r = csi[1].r, cdi[0].i = csi[1].i;
    i__1 = *nm;
    for (k = 1; k <= i__1; ++k) {
/* L25: */
	i__3 = k;
	...
	d__1 = k + 1.;
	i__4 = k;
	z__3.r = d__1 * csi[i__4].r, z__3.i = d__1 * csi[i__4].i;
	z_div(&z__2, &z__3, z__);
	z__1.r = csi[i__2].r - z__2.r, z__1.i = csi[i__2].i - z__2.i;
    ...
    z__1.r = z__2.r * z__4.r - z__2.i * z__4.i, z__1.i = z__2.r * z__4.i + 
	    z__2.i * z__4.r;
    csk[0].r = z__1.r, csk[0].i = z__1.i;
    z_div(&z__3, &c_b6, z__);
    z__2.r = z__3.r + 1., z__2.i = z__3.i;
    z__1.r = csk[0].r * z__2.r - csk[0].i * z__2.i, z__1.i = csk[0].r * 
	    z__2.i + csk[0].i * z__2.r;
    csk[1].r = z__1.r, csk[1].i = z__1.i;
    i__3 = *nm;
    for (k = 2; k <= i__3; ++k) {
	if (z_abs(&csi[k - 1]) > z_abs(&csi[k - 2])) {
	    i__2 = k;
	    d__1 = pi * .5;
	    z__4.r = d__1, z__4.i = 0.;
	    z__5.r = z__->r * z__->r - z__->i * z__->i, z__5.i = z__->r * 
	    ...
	    i__4 = k;
	    i__1 = k - 1;
	    z__6.r = csi[i__4].r * csk[i__1].r - csi[i__4].i * csk[i__1].i, 
		    z__6.i = csi[i__4].r * csk[i__1].i + csi[i__4].i * csk[
		    i__1].r;
	    z__2.r = z__3.r - z__6.r, z__2.i = z__3.i - z__6.i;
	    z_div(&z__1, &z__2, &csi[k - 1]);
	    csk[i__2].r = z__1.r, csk[i__2].i = z__1.i;
	} else {
	    i__2 = k;
	    i__4 = k;
	    i__1 = k - 2;
	    z__3.r = csi[i__4].r * csk[i__1].r - csi[i__4].i * csk[i__1].i, 
		    z__3.i = csi[i__4].r * csk[i__1].i + csi[i__4].i * csk[
		    i__1].r;
	    d__1 = (k - .5) * pi;
	    z__5.r = d__1, z__5.i = 0.;
	    pow_zi(&z__6, z__, &c__3);
	    z_div(&z__4, &z__5, &z__6);
	    z__2.r = z__3.r + z__4.r, z__2.i = z__3.i + z__4.i;
	    z_div(&z__1, &z__2, &csi[k - 2]);
	    csk[i__2].r = z__1.r, csk[i__2].i = z__1.i;
	}
/* L30: */
    }
    z__1.r = -csk[1].r, z__1.i = -csk[1].i;
...
/* L35: */
	i__2 = k;
	i__4 = k - 1;
	z__2.r = -csk[i__4].r, z__2.i = -csk[i__4].i;
	d__1 = k + 1.;
	i__1 = k;
	z__4.r = d__1 * csk[i__1].r, z__4.i = d__1 * csk[i__1].i;
	z_div(&z__3, &z__4, z__);
	z__1.r = z__2.r - z__3.r, z__1.i = z__2.i - z__3.i;
	cdk[i__2].r = z__1.r, cdk[i__2].i = z__1.i;
    }
    return 0;
} /* csphik_ */

/*       ********************************** */
/* Subroutine */ int bjndd_(n, x, bj, dj, fj)
integer *n;
doublereal *x, *bj, *dj, *fj;
{
    /* System generated locals */
    integer i__1;
    real r__1;
...
/*       ===================================================== */
    ...
    for (nt = 1; nt <= 900; ++nt) {
	r__1 = nt * (float)6.28;
	d__1 = abs(*x) * (float)1.36 / nt;
	mt = (integer) (r_lg10(&r__1) * (float).5 - nt * d_lg10(&d__1));
	if (mt > 20) {
	    goto L15;
	}
/* L10: */
...
/*       ********************************** */
/* Subroutine */ int sphj_(n, x, nm, sj, dj)
integer *n;
doublereal *x;
integer *nm;
doublereal *sj, *dj;
...
/*       ********************************** */
/* Subroutine */ int othpl_(kf, n, x, pl, dpl)
integer *kf, *n;
doublereal *x, *pl, *dpl;
{
    /* System generated locals */
...
/*       ********************************** */
/* Subroutine */ int klvnzo_(nt, kd, zo)
integer *nt, *kd;
doublereal *zo;
{
    /* System generated locals */
...
/*       ********************************** */
/* Subroutine */ int rswfo_(m, n, c__, x, cv, kf, r1f, r1d, r2f, r2d)
integer *m, *n;
doublereal *c__, *x, *cv;
integer *kf;
doublereal *r1f, *r1d, *r2f, *r2d;
{
    ...
    static doublereal df[200];
    static integer id, kd;
    extern /* Subroutine */ int rmn2so_(), rmn1_();
...
/*       ********************************** */
/* Subroutine */ int ch12n_(n, z__, nm, chf1, chd1, chf2, chd2)
integer *n;
doublecomplex *z__;
integer *nm;
doublecomplex *chf1, *chd1, *chf2, *chd2;
{
    /* System generated locals */
    integer i__1, i__2, i__3, i__4;
    doublecomplex z__1, z__2, z__3;
    ...
    /* Local variables */
    static integer k;
    extern /* Subroutine */ int ciknb_(), cjynb_();
    static doublecomplex ci;
    static doublereal pi;
...
/*       ********************************** */
/* Subroutine */ int jyzo_(n, nt, rj0, rj1, ry0, ry1)
integer *n, *nt;
doublereal *rj0, *rj1, *ry0, *ry1;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1, d__2;
    ...
    static integer l;
    static doublereal x;
    extern /* Subroutine */ int jyndd_();
    static doublereal x0, pi, xguess, bjn, djn, fjn, byn, dyn, fyn;
...
/*       1) initial guess for j_{N,1} */
    if (*n <= 20) {
	x = *n * (float)1.15859 + (float)2.82141;
    } else {
/*          Abr & Stg (9.5.14) */
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1307) * (float)1.85576 + (float)1.03315 / 
		pow_dd(&d__2, &c_b1307);
    }
    l = 0;
/*       2) iterate */
...
/* Computing 2nd power */
    i__1 = *n;
    d__1 = (*n * (float).0679 + .0972 - i__1 * i__1 * (float)3.54e-4) / l;
    x = x + pi + max(d__1,0.);
    if (l < *nt) {
	goto L10;
    }
/*       -- Newton method for j_{N,L}' */
    if (*n <= 20) {
	x = *n * (float)1.07703 + (float).961587;
    } else {
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1307) * (float).80861 + (float).07249 / 
		pow_dd(&d__2, &c_b1307);
    }
    if (*n == 0) {
	x = (float)3.8317;
...
/* Computing 2nd power */
    i__1 = *n;
    d__1 = (*n * (float).0915 + .4955 - i__1 * i__1 * (float)4.35e-4) / l;
    x = x + pi + max(d__1,0.);
    if (l < *nt) {
	goto L15;
    }
/*       -- Newton method for y_{N,L} */
    if (*n <= 20) {
	x = *n * (float)1.08933 + (float)1.19477;
    } else {
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1307) * (float).93158 + (float).26035 / 
		pow_dd(&d__2, &c_b1307);
    }
    l = 0;
    xguess = x;
...
/* Computing 2nd power */
    i__1 = *n;
    d__1 = (*n * (float).0852 + .312 - i__1 * i__1 * (float)4.03e-4) / l;
    x = x + pi + max(d__1,0.);
    if (l < *nt) {
	goto L20;
    }
/*       -- Newton method for y_{N,L}' */
    if (*n <= 20) {
	x = *n * (float)1.16099 + (float)2.67257;
    } else {
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1307) * (float)1.8211 + (float).94001 / 
		pow_dd(&d__2, &c_b1307);
    }
    l = 0;
    xguess = x;
...
/* Computing 2nd power */
    i__1 = *n;
    d__1 = (*n * (float).0643 + .197 - i__1 * i__1 * (float)2.86e-4) / l;
    x = x + pi + max(d__1,0.);
    if (l < *nt) {
	goto L25;
    }
...
/*       ********************************** */
/* Subroutine */ int ikv_(v, x, vm, bi, di, bk, dk)
doublereal *v, *x, *vm, *bi, *di, *bk, *dk;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1;
    ...
    /* Builtin functions */
    double pow_dd(), exp(), sqrt(), log(), sin();
    ...
    static doublereal r1;
    extern /* Subroutine */ int gamma2_();
    static doublereal r2, v0, w0, x2, ca, cb, cs, ct, wa, pi, vt, ww, bi0, 
...
/*       ********************************** */
/* Subroutine */ int sdmn_(m, n, c__, cv, kd, df)
integer *m, *n;
doublereal *c__, *cv;
integer *kd;
doublereal *df;
...
/* L5: */
	...
	a[i__ - 1] = (d2k + (float)2.) * (d2k + (float)1.) / ((dk2 + (float)
		3.) * (dk2 + (float)5.)) * cs;
	d__[i__ - 1] = dk0 * dk1 + (dk0 * (float)2. * dk1 - *m * (float)2. * *
		m - (float)1.) / ((dk2 - (float)1.) * (dk2 + (float)3.)) * cs;
	g[i__ - 1] = k * (k - (float)1.) / ((dk2 - (float)3.) * (dk2 - (float)
		1.)) * cs;
/* L10: */
    }
    fs = 1.;
...
/*       ********************************** */
/* Subroutine */ int ajyik_(x, vj1, vj2, vy1, vy2, vi1, vi2, vk1, vk2)
doublereal *x, *vj1, *vj2, *vy1, *vy2, *vi1, *vi2, *vk1, *vk2;
{
    /* System generated locals */
    integer i__1;
    doublereal d__1, d__2;
    ...
    /* Builtin functions */
    double pow_dd(), sqrt(), cos(), sin(), exp();
...
/*       ********************************** */
/* Subroutine */ int cikvb_(v, z__, vm, cbi, cdi, cbk, cdk)
doublereal *v;
doublecomplex *z__;
doublereal *vm;
doublecomplex *cbi, *cdi, *cbk, *cdk;
{
    /* System generated locals */
    integer i__1, i__2, i__3, i__4;
    doublereal d__1, d__2, d__3;
    ...
    /* Builtin functions */
    double z_abs();
    void pow_zz(), z_div(), z_exp(), z_sqrt();
    double pow_dd();
    void z_log();
    double sin(), d_imag();
...
/*       ********************************** */
/* Subroutine */ int cikva_(v, z__, vm, cbi, cdi, cbk, cdk)
doublereal *v;
doublecomplex *z__;
doublereal *vm;
doublecomplex *cbi, *cdi, *cbk, *cdk;
{
    ...
    /* Builtin functions */
    double z_abs();
    void pow_zz(), z_exp(), z_sqrt(), z_div();
    double pow_dd();
    void z_log();
    double sin(), d_imag();
...
/* L20: */
	...
	m = (integer) (t * (float)1.75 + (float)42.);
	su = 0.;
	*c__ = 0.;
	*s = 0.;
	f1 = 0.;
...
/*       ********************************** */
/* Subroutine */ int rctj_(n, x, nm, rj, dj)
integer *n;
doublereal *x;
integer *nm;
doublereal *rj, *dj;
...
/*       ********************************** */
/* Subroutine */ int jy01b_(x, bj0, dj0, bj1, dj1, by0, dy0, by1, dy1)
doublereal *x, *bj0, *dj0, *bj1, *dj1, *by0, *dy0, *by1, *dy1;
{
    /* Builtin functions */
    double log(), sqrt(), cos(), sin();
...
/*       ********************************** */
/* Subroutine */ int sphk_(n, x, nm, sk, dk)
integer *n;
doublereal *x;
integer *nm;
doublereal *sk, *dk;
...
/*       ===================================================== */
    ...
    if (*x == (doublereal) ((integer) (*x)) && *x > (float)0.) {
	*ga = 1.;
	m1 = (integer) (*x - (float)1.);
	i__1 = m1;
	for (k = 2; k <= i__1; ++k) {
/* L10: */
	    *ga *= k;
	}
    } else if (*x + .5 == (doublereal) ((integer) (*x + .5)) && *x > (float)
	    0.) {
	m = (integer) (*x);
	*ga = sqrt(pi);
	i__1 = m;
...
/*       ********************************** */
/* Subroutine */ int pbvv_(v, x, vv, vp, pvf, pvd)
doublereal *v, *x, *vv, *vp, *pvf, *pvd;
{
    /* System generated locals */
    integer i__1;

    /* Builtin functions */
    double d_sign(), exp(), sqrt();

    /* Local variables */
    extern /* Subroutine */ int vvla_(), vvsa_();
    static doublereal f;
    static integer k, l, m;
    static doublereal f0, f1, s0, v0, v1, v2;
    static integer ja, na;
...
/*       ********************************** */
/* Subroutine */ int clqmn_(mm, m, n, x, y, cqm, cqd)
integer *mm, *m, *n;
doublereal *x, *y;
doublecomplex *cqm, *cqd;
{
    /* System generated locals */
...
/* L20: */
	...
	if (xc > (float)1.1) {
	    ...
	    km = (*m + 40 + *n) * (integer) ((float)-1. - log(xc - (float)1.) 
		    * (float)1.8);
	}
	cqf2.r = 0., cqf2.i = 0.;
	cqf1.r = 1., cqf1.i = 0.;
...
/*       ********************************** */
/* Subroutine */ int segv_(m, n, c__, kd, cv, eg)
integer *m, *n;
doublereal *c__;
integer *kd;
doublereal *cv, *eg;
...
/* L5: */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    ...
	    dk2 = (doublereal) (*m + k << 1);
	    d2k = (doublereal) ((*m << 1) + k);
	    a[i__ - 1] = (d2k + (float)2.) * (d2k + (float)1.) / ((dk2 + (
		    float)3.) * (dk2 + (float)5.)) * cs;
	    d__[i__ - 1] = dk0 * dk1 + (dk0 * (float)2. * dk1 - *m * (float)
		    2. * *m - (float)1.) / ((dk2 - (float)1.) * (dk2 + (float)
		    3.)) * cs;
/* L10: */
	    g[i__ - 1] = k * (k - (float)1.) / ((dk2 - (float)3.) * (dk2 - (
		    float)1.)) * cs;
	}
	i__1 = nm;
	for (k = 2; k <= i__1; ++k) {
...
/*       ********************************** */
/* Subroutine */ int ciknb_(n, z__, nm, cbi, cdi, cbk, cdk)
integer *n;
doublecomplex *z__;
integer *nm;
doublecomplex *cbi, *cdi, *cbk, *cdk;
{
    ...
    /* Builtin functions */
    double z_abs();
    void z_div(), z_exp(), z_log(), z_sqrt();
    double d_imag();

    /* Local variables */
    static doublecomplex cbkl;
    extern integer msta1_(), msta2_();
    static integer k, m, l;
    static doublereal a0;
    static integer k0;
...
/*       ********************************** */
/* Subroutine */ int cikna_(n, z__, nm, cbi, cdi, cbk, cdk)
integer *n;
doublecomplex *z__;
integer *nm;
doublecomplex *cbi, *cdi, *cbk, *cdk;
...
/*       ********************************** */
/* Subroutine */ int mtu12_(kf, kc, m, q, x, f1r, d1r, f2r, d2r)
integer *kf, *kc, *m;
doublereal *q, *x, *f1r, *d1r, *f2r, *d2r;
{
    /* System generated locals */
    integer i__1, i__2;
    doublereal d__1;
...
/*       ********************************** */
/* Subroutine */ int cik01_(z__, cbi0, cdi0, cbi1, cdi1, cbk0, cdk0, cbk1, 
	cdk1)
doublecomplex *z__, *cbi0, *cdi0, *cbi1, *cdi1, *cbk0, *cdk0, *cbk1, *cdk1;
{
    /* Initialized data */
    ...
    /* Builtin functions */
    double z_abs();
    void z_div(), z_exp(), z_sqrt(), pow_zi(), z_log();
    double d_imag();
...
/*       ********************************** */
/* Subroutine */ int cpsi_(x, y, psr, psi)
doublereal *x, *y, *psr, *psi;
{
    /* Initialized data */
    ...
    /* Builtin functions */
    double atan(), sqrt(), log(), cos(), pow_di(), sin(), pow_dd(), tan(), 
	    tanh();
...
/*       ********************************** */
/* Subroutine */ int sphy_(n, x, nm, sy, dy)
integer *n;
doublereal *x;
integer *nm;
doublereal *sy, *dy;
...
/*       ********************************** */
/* Subroutine */ int jelp_(u, hk, esn, ecn, edn, eph)
doublereal *u, *hk, *esn, *ecn, *edn, *eph;
{
    /* System generated locals */
    doublereal d__1;

    /* Builtin functions */
    double sqrt(), pow_di(), sin(), atan(), cos();
...
doublereal *v, *x, *hv;
{
    ...
    /* Builtin functions */
    integer pow_ii();
    double sqrt(), pow_dd(), sin(), cos();

    /* Local variables */
    static integer k, l, n;
    static doublereal s, u, r1;
    extern /* Subroutine */ int gamma2_();
    static doublereal r2, v0, s0, u0, t0, t1, bf, ga, gb, sa, va, vb, pi, sr, 
	    vt, bf0, bf1, bj0, bj1, by0, by1, pu0, qu0, pu1, qu1, bjv, byv;


/*       ===================================================== */
/*       Purpose: Compute Struve function Hv(x) with an */
/*                arbitrary order v */
/*       Input :  v  --- Order of Hv(x)  ( -8.0 ≤ v ≤ 12.5 ) */
/*                x  --- Argument of Hv(x) ( x ≥ 0 ) */
/*       Output:  HV --- Hv(x) */
/*       Note: numerically unstable away from the above range for `v` */
/*       Routine called: GAMMA2 to compute the gamma function */
/*       ===================================================== */
    ...
    pu0 = 0.;
    if (*x <= 20.) {
/*          Power series for Hv (Abramowitz & Stegun 12.1.3) */
	v0 = *v + 1.5;
	gamma2_(&v0, &ga);
	s = 2. / (sqrt(pi) * ga);
	r1 = 1.;
...
L15:
	d__1 = *x * .5;
	d__2 = *v + 1.;
	*hv = pow_dd(&d__1, &d__2) * s;
    } else {
/*          Asymptotic large |z| expansion for Hv - Yv  (Abm & Stg 12.1.29) */
	d__1 = *x * .5;
	d__2 = *v - (float)1.;
	sa = pow_dd(&d__1, &d__2) / pi;
	v0 = *v + .5;
	gamma2_(&v0, &ga);
	s = sqrt(pi) / ga;
	r1 = 1.;
...
/* Computing 2nd power */
		d__2 = k * 4. - (float)1.;
		r1 = r1 * -.0078125 * (vt - d__1 * d__1) * (vt - d__2 * d__2) 
			/ ((k * 2. - (float)1.) * k * *x * *x);
		pu1 += r1;
/* L25: */
	    }
...
/* Computing 2nd power */
		d__2 = k * 4. + (float)1.;
		r2 = r2 * -.0078125 * (vt - d__1 * d__1) * (vt - d__2 * d__2) 
			/ ((k * 2. + (float)1.) * k * *x * *x);
		qu1 += r2;
/* L30: */
	    }
...
/* L35: */
	...
	t0 = *x - (u0 * (float).5 + .25) * pi;
	t1 = *x - (u0 * (float).5 + .75) * pi;
	sr = sqrt(2. / (pi * *x));
	by0 = sr * (pu0 * sin(t0) + qu0 * cos(t0));
	by1 = sr * (pu1 * sin(t1) + qu1 * cos(t1));
/*          Compute Y_|v|   (Abm & Stg 9.1.27) */
	bf0 = by0;
	bf1 = by1;
	i__1 = n;
	for (k = 2; k <= i__1; ++k) {
...
/*          Compute Y_v  (handle the case v < 0 appropriately) */
	if (*v < 0.) {
	    if (u0 == 0.) {
/*                Use symmetry (Abm & Stg 9.1.5) */
		byv = pow_ii(&c_n1, &n) * byv;
	    } else {
/*                Use relation between Yv & Jv (Abm & Stg 9.1.6) */
/*                Compute J_(|v|-N) (Abm & Stg 9.2.5) */
		bj0 = sr * (pu0 * cos(t0) - qu0 * sin(t0));
		bj1 = sr * (pu1 * cos(t1) - qu1 * sin(t1));
/*                Forward recurrence for J_|v| (Abm & Stg 9.1.27) */
/*                It's OK for the limited range -8.0 ≤ v ≤ 12.5, */
/*                since x >= 20 here; but would be unstable for v <~ -20 */
		bf0 = bj0;
		bf1 = bj1;
		i__1 = n;
		for (k = 2; k <= i__1; ++k) {
...
/*                Compute Y_v    (Abm & Stg 9.1.6) */
		byv = cos(*v * pi) * byv + sin(-(*v) * pi) * bjv;
	    }
	}
/*          Compute H_v */
	*hv = byv + s0;
